...
def bfgs_e(func, grad, x0, eps_f=0.0, eps_g=0.0, callback=None, options=None):
    ...
    if (not np.any(eps_g) and callable(grad)) or eps_f == 0.0:
        eps_g = 0.0
    ...
    while iter < options["max_iter"]:
        ...
        if options["display"] >= 2 and iter % 25 == 0:
            ...
            print(
                "    Iter      |      F       |    ||g||     |   F Evals    |   G Evals    |    alpha     |     beta     |      mu      "
            )
            print(
                "-----------------------------------------------------------------------------------------------------------------------"
            )
        ...
        (
            ...
            beta_init=options["beta_init"],
            mu=mu,
            c1=options["c1"],
            c2=options["c2"],
            c3=options["c3"],
            split_iter=options["split_iter"],
            max_ls_iter=options["max_ls_iter"],
            verbose=verbose,
        )
        ...
        if wolfe_flag:
            ...
            if options["qn_hist_size"] < np.inf:
                ...
                H = np.matmul(np.matmul(mat, H), mat.transpose()) + rho * np.outer(
                    s_k, s_k
                )
        ...
        # check if gradient noise level is reached
            ...
            options["terminate"] >= 2
            and isinstance(eps_gk, np.ndarray)
            and eps_gk.ndim > 0
            and eps_gk.shape[0] > 1
            and np.all(np.abs(g_k) < eps_gk)
        ):
            flag = 5
            break
            ...
            options["terminate"] >= 2
            and (
                not isinstance(eps_gk, np.ndarray)
                or eps_gk.ndim == 0
                or eps_gk.shape[0] == 0
            )
            and norm_gk < eps_gk
        ...
        # check numerics
            ...
            and eps_g == 0
            and (
                np.linalg.norm(x_k - x_old, ord=np.inf)
                < eps_m * np.maximum(1.0, np.linalg.norm(x_k, ord=np.inf))
                or f_old - f_k < eps_m * np.maximum(1.0, np.abs(f_k))
            )
        ):
            flag = 7
            break
...
def _line_search_nt_wolfe(
    ...
    if callable(eps_g):
        ...
        eps_gk = np.copy(eps_g)
    eps_gp = None  # Added by Hiroki Hamaguchi to fix bug
    ...
    if not armijo_flag or not wolfe_flag or split_flag:
        ...
        if f_best < np.inf:
            ...
            alpha, f_new, eps_fp, fevals_back, armijo_flag = _backtracking(
                func,
                x_k,
                p_k,
                g_k,
...
def _lengthening(
    ...
    while ls_iter < max_ls_iter:
        ...
        if verbose:
            print(
                "  % 8.3e  |  % 8.3e  |  % 8.3e  |  % 8.3e  |  % 8.3e  |  % 8.3e  |  % 8.3e  "
...
if __name__ == "__main__":
    ...
    x_opt, f_opt, iters, f_evals, g_evals, flag, results = bfgs_e(
        func, grad, x0, options=options
    )
    ...
    def grad(x):
        return scipy.optimize.rosen_der(x) + np.random.uniform(
            -1e-5, 1e-5, size=x0.shape
        )

    options = {"qn_hist_size": np.inf}
    x_opt, f_opt, iters, f_evals, g_evals, flag, results = bfgs_e(
        func, grad, x0, eps_f=1e-5, eps_g=np.sqrt(2) * 1e-5, options=options
    )
