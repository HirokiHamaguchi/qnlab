...
from qnlab.solver.external_files.kanzow.utility import (
    limitedMemory,
    parameters,
    regularization,
)
...
def solveNonmonotone(f, Df, x):
    ...
    lmData = limitedMemory.NormalizedLmData(x.shape[0], parameters.memory)
    return regularization.genericNonmonotone(
        lmData, updateLmData, inverseLBFGS, f, Df, x
    )
...
def inverseLBFGS(data, mu, g):
    ...
    Q21 = np.triu(data.SnTYn[:mUpd, :mUpd]) - mu / gamma * np.tril(
        data.SnTYn[:mUpd, :mUpd], -1
    )
    ...
    p = scipy.linalg.solve_triangular(
        Qfactor2, scipy.linalg.solve_triangular(Qfactor1, ATg, lower=True)
    )
