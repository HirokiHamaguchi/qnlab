...
\ifEn
    Line-search-based quasi-Newton methods generate a sequence $\{ x_k \}_{k=0}^{\infty}$ that converges to the optimal solution $x^*$ as follows:
\else
    直線探索に基づく準ニュートン法では、最適解 $x^*$ に収束する列 $\{ x_k \}_{k=0}^{\infty}$ を次のように生成します。
\fi
\begin{equation*}
