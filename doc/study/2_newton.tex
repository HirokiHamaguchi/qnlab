...
\end{equation*}
\ifEn
    In this section, we outline Newton's method, which is a fundamental optimization algorithm for this unconstrained optimization problem.
\else
...
\end{equation*}
\ifEn
    Here, $\alpha_k > 0$ is the step size determined by line search.
    We can derive this update rule as follows.
    The quadratic Taylor approximation of $f$ at $x_k$ is given by
\else
...
\end{equation*}
\ifEn
    Selecting $x^*$ directly as the next iterate may seem natural, but as we will see later, it does not generally guarantee global convergence.
    Thus, we introduce a step size $\alpha_k > 0$ determined by line search to ensure a sufficient decrease in function values, leading to the update rule presented initially.
...
\end{equation*}
\ifEn
    When $f$ is nonconvex, meaning that the Hessian $\nabla^2 f(x_k)$ is negative definite or indefinite, it may not be a descent direction, and Newton's method may fail to converge.
    The modified Newton's method~\citep[Sec. 3.4]{nocedal1999numerical} is one remedy for handling such cases, but we omit the explanation here.
\else
...
\ifEn
    ...
    The second condition is known as the curvature condition, which requires that the rate of decrease projected onto the search direction at the next point $-g_{k+1}^\top d_k$ is at most a fraction $c_2$ of the current projected decrease $-g_k^\top d_k$, indicating sufficient optimization along the search direction $d_k$.
    The Wolfe conditions refer to the simultaneous satisfaction of these two conditions.
...
\begin{theorem}[{\citep[Theorem 3.2]{nocedal1999numerical}}]
    ...
    \ifEn
        Suppose that $f \colon \mathbb{R}^n \to \mathbb{R}$ is of class $C^1$ and bounded below in $\mathbb{R}^n$, and $L$-smooth.
    ...
    \end{equation*}
    \ifEn
        It starts from an initial point $x_0 \in \mathbb{R}^n$, and the step size $\alpha_k$ satisfies the Wolfe conditions.
        For the angles $\{ \theta_k \}_k$, if there exists a positive constant $\delta$ such that $\cos \theta_k \geq \delta > 0$ for all $k$,
        then, the method generates a sequence $\{ x_k \}_k$ satisfying
    \else
        初期点 $x_0 \in \mathbb{R}^n$ から開始され、ステップサイズ $\alpha_k$ は Wolfe 条件を満たすとする。
        角 $\{ \theta_k \}_k$ に対して、ある正の定数 $\delta$ が存在して $\cos \theta_k \geq \delta > 0$ が全ての $k$ で成り立つならば、
        この反復法は次を満たす点列 $\{ x_k \}_k$ を生成する。
    \fi
    \begin{equation*}
        \lim_{k \to \infty} \left\lVert g_k \right\rVert = 0.
    \end{equation*}
...
\begin{proof}
    ...
    \ifEn
        ...
        If the initial point is chosen such that $\left\lVert x_0-x^* \right\rVert\le \min\{ r, 1/(2\widetilde L) \}$, then by induction $\{ x_k \}$ remains within the neighborhood and converges to $x^*$.
    ...
    \else
        ...
        初期点が $\left\lVert x_0-x^* \right\rVert\le \min\{ r, 1/(2\widetilde L) \}$ を満たすように選ばれていれば、帰納法により $\{ x_k \}$ は近傍内に留まり $x^*$ に収束する。
...
\end{proof}
\ifEn
    These \cref{thm:line-search-global-convergence,thm:newton-quadratic} indicate that Newton's method equipped with line search has both global convergence and local quadratic convergence properties under appropriate conditions.
    ...
    However, computing the Hessian $\nabla^2 f(x_k) \in \mathbb{R}^{n \times n}$ and solving the linear system $\nabla^2 f(x_k) d_k = -\nabla f(x_k)$ can be computationally expensive, especially for large-scale problems.
    Therefore, other optimization methods, such as quasi-Newton methods, are often employed in practice, which we will discuss in later sections.
\else
...
\ifEn
    ...
    When the Hessian matrix is positive definite, Newton's method provides a descent direction, since
\else
...
\end{equation*}
\ifEn
    In contrast, when the Hessian matrix is negative definite or indefinite, a decrease in the function value is not guaranteed, and Newton's method can move away from the optimal solution.
    In the indefinite case, the function value may decrease, but the risk of converging to a saddle point is also increased.
    Therefore, the positive definiteness of the Hessian matrix is important when applying Newton's method.
\else
...
\ifEn
    ...
    Here, we present specific examples of such failures and discuss the necessity of line search as a countermeasure.
\else
...
\ifEn
    From these examples, it is clear that adopting a full step size $\alpha_k=1$ in Newton's method can lead to divergence or oscillation, even for strongly convex and smooth functions.
    Therefore, it is essential to select the step size $\alpha_k$ appropriately to ensure convergence.
    Newton's method equipped with line search is often referred to as the modified Newton's method.
\else
...
\ifEn
    ...
    Although this is not the main topic of this section, it is arguably fundamental and widely known.
\else
