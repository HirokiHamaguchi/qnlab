...
\else
    ニュートン法は、初期点 $x_0$ から出発し、現在の点を逐次更新して点列 $\{ x_k \}_{k=0}^\infty$ を生成する代表的な反復アルゴリズムです。
\fi
\ifEn
    ...
    Using the gradient $g_k \coloneqq \nabla f(x_k)$ and Hessian $\nabla^2 f(x_k)$ at the $k$-th iterate $x_k$, the base procedure of Newton's method is to iteratively update as
\else
...
\end{equation*}
\ifEn
    Here, $\alpha_k > 0$ is the step size determined by line search. We can derive this update rule as follows. The quadratic Taylor approximation of $f$ at $x_k$ is given by
\else
...
\else
    次の反復点として、直接 $x^*$ を選ぶことは自然に思えますが、後述するようにこのままでは大域収束性を保証できません。そこで、関数値の十分な減少を確保するため、直線探索で定めるステップサイズ $\alpha_k > 0$ を導入すると、最初に示した更新式が得られます。これがニュートン法の基本的な手続きとなります。
\fi
...
\ifEn
    In Newton's method, adopting a step size $\alpha_k=1$ at every iteration may lead to difficulties. Here, we present specific examples of such failures and discuss the necessity of line search as a countermeasure.
\else
...
\ifEn
    As a conceptual supplement, we briefly clarify the relationship between “Newton's method as a root-finding algorithm” and “Newton's method in optimization.” These two formulations are closely related, but they have different perspectives and applications.
\else
...
\ifEn
    When referring simply to Newton's method (or the Newton–Raphson method), one may mean a root-finding algorithm for a differentiable scalar function $g\colon \mathbb{R} \to \mathbb{R}$ that solves $g(x) = 0$. It is also arguably fundamental and widely known.
\else
...
\ifEn
    Let us examine the equivalence of the two formulations through a concrete example. \Cref{fig:newton_raphson} illustrates the relationship between these formulations using a simple cubic function $f(x)$. The left panel shows root-finding applied to $g(x) = \nabla f(x)$, while the right panel shows optimization applied to $f(x)$.
\else
    具体例を通じて両者の等価性を確認してみます。\Cref{fig:newton_raphson}は単純な三次関数 $f(x)$ を用いて両者の関係を示したものです。左図は $g(x) = \nabla f(x)$ に対する根探索であり、右図は $f(x)$ に対する最適化となっています。
\fi
\ifEn
