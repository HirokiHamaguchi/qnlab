...
\ifEn
    We define $f \colon \mathbb{R}^n \to \mathbb{R}$ as an objective function of class $C^2$, where $n$ denotes the dimension of the decision variables. In this context, the following unconstrained optimization problem is one of the most fundamental optimization problems:
\else
...
\else
    よって、次の反復点としては、この $x^*$ を選ぶことが自然で、これがニュートン法の更新則となります。
\fi
...
\end{equation*}
\ifEn
    This variant is called the \href{https://en.wikipedia.org/wiki/Newton%27s_method_in_optimization#Higher_dimensions}{damped Newton's method} or relaxed Newton's method, and it is a common practice to use this version in general optimization problems.
\else
    この亜種は\href{https://ja.wikipedia.org/wiki/%E6%9C%80%E9%81%A9%E5%8C%96%E3%81%AB%E3%81%8A%E3%81%91%E3%82%8B%E3%83%8B%E3%83%A5%E3%83%BC%E3%83%88%E3%83%B3%E6%B3%95#%E9%AB%98%E6%AC%A1%E5%85%83}{減衰ニュートン法(damped Newton's method)}あるいは緩和ニュートン法(relaxed Newton's method)と呼ばれます(個人的にはdampedの方がよく聞きます)。
\fi
...
\end{equation*}
\ifEn
    The direction $d_k$ is a descent direction satisfying $g_k^\top d_k < 0$ and $\alpha_k > 0$ is the step size determined by line search. We employ the (weak) Wolfe conditions~\citep[Sec. 3.1]{nocedal1999numerical} to determine the step size $\alpha_k$ as follows:
\else
    ここで $d_k$ は $g_k^\top d_k < 0$ を満たす降下方向であり、$\alpha_k > 0$ は直線探索で決定されるステップサイズです。ステップサイズ $\alpha_k$ は (弱)Wolfe 条件~\citep[Sec. 3.1]{nocedal1999numerical} により次のように定められるとします。
\fi
\begin{equation*}
    \begin{dcases}
...
\ifEn
    ...
    The Wolfe conditions refer to the simultaneous satisfaction of Armijo and curvature conditions. \Cref{fig:armijo_wolfe_conditions} illustrates the ranges where these conditions are satisfied.
\else
    $0 < c_1 < c_2 < 1$ は定数です。
    ...
    Wolfe条件とは、Armijo 条件と曲率条件の両方が同時に満たされることを指します。\Cref{fig:armijo_wolfe_conditions} にこれらの条件が満たされる範囲を図示しています。
\fi

\begin{figure}[t]
    \centering
    \includegraphics[width=0.8\columnwidth]{../imgs/quasi_newton/armijo_wolfe_conditions.pdf}
    \caption{
        \ifEn
...
\begin{proof}
    ...
    \else
        仮定より $f$ は下に有界なので、ある定数 $f^*$ が存在して、全ての $k$ について $f(x_{k+1}) \geq f^*$ となります。従って、Zoutendijk 条件と呼ばれる次式が得られます。
    \fi
    \begin{equation*}
...
\ifEn
    Since damped Newton's method is a special case of the setting in \cref{thm:line-search-global-convergence} with $d_k = -\nabla^2 f(x_k)^{-1} g_k$, we can apply this result under appropriate conditions of $f$. In particular, if $f$ is $L$-smooth and $\mu$-strongly convex, then for any $k$, we have
\else
    減衰ニュートン法は $d_k = -\nabla^2 f(x_k)^{-1} g_k$ とした \cref{thm:line-search-global-convergence} の特別な場合なので、適切な $f$ に関する条件の下でこの結果を適用できます。特に $f$ が $L$-平滑かつ $\mu$-強凸であれば、任意の $k$ について次が成り立ちます。
\fi
\begin{equation*}
    \cos \theta_k
...
\end{equation*}
\ifEn
    Here, $\lambda_{\min}(\cdot)$ and $\lambda_{\max}(\cdot)$ denote the minimum and maximum eigenvalues, respectively. Thus, by setting $\delta = \mu / L$ in \cref{thm:line-search-global-convergence}, we obtain the global convergence of damped Newton's method for strongly convex and smooth functions under line search satisfying the Wolfe conditions.
\else
    ここで $\lambda_{\min}(\cdot)$ と $\lambda_{\max}(\cdot)$ はそれぞれ最小固有値と最大固有値を表すとします。従って、\cref{thm:line-search-global-convergence} において $\delta = \mu / L$ とすれば、Wolfe 条件を満たす直線探索の下で、強凸かつ平滑な関数に対する減衰ニュートン法の大域収束性が得られます。
\fi
...
\begin{theorem}[{\citep[Theorem 3.5]{nocedal1999numerical}}]
    ...
    \ifEn
        ...
        In Newton's method, if $\alpha_k=1$ holds for all $k$ and the initial point $x_0$ is sufficiently close to $x^*$, then the sequence of gradient norms $\{\left\lVert \nabla f(x_k) \right\rVert\}_k$ converges quadratically.
    \else
        ヘッセ行列 $\nabla^2 f(x)$ が解 $x^*$ の近傍でリプシッツ連続であり、最適性の二次の十分条件が成り立つとする。すなわち $\nabla f(x^*)=0$ かつ $\nabla^2 f(x^*)$ は正定値である。
        ニュートン法において、すべての $k$ で $\alpha_k=1$ が満たされるとし、初期点 $x_0$ が $x^*$ に十分近いとき、勾配ノルム列 $\{\left\lVert \nabla f(x_k) \right\rVert\}_k$ は二次収束する。
    \fi
\end{theorem}
\begin{proof}
    \ifEn
    ...
    \else
        $\nabla^2 f$ が定数 $L^{\mathrm{H}}$ においてリプシッツ連続であれば、被積分関数は $L^{\mathrm{H}}t\left\lVert x_k-x^* \right\rVert$ で上から抑えられ、積分により次を得ます。
    \fi
    \begin{equation*}
        \left\lVert \nabla^2 f(x_k)(x_k-x^*)-\left(\nabla f(x_k)-\nabla f(x^*)\right) \right\rVert
    ...
    \else
        $\nabla^2 f(x^*)$ は正定値でありリプシッツ連続なので、ある半径 $r>0$ が存在して $\left\lVert x-x^* \right\rVert\le r$ を満たすすべての $x$ について
    \fi
    \begin{equation*}
    ...
    \else
        $\widetilde L \coloneqq L^{\mathrm{H}}\left\lVert \left(\nabla^2 f(x^*)\right)^{-1} \right\rVert$ とおきます。初期点が $\left\lVert x_0-x^* \right\rVert\le \min\{ r, 1/(2\widetilde L) \}$ を満たすように選ばれていれば、帰納法により $\{ x_k \}_k$ は近傍内に留まり $x^*$ に収束します。よって、上の誤差評価は $\{ x_k \}_k$ の二次収束を示します。
...
\end{proof}
\ifEn
    These \cref{thm:line-search-global-convergence,thm:newton-quadratic} indicate that Newton's method has both global convergence and local quadratic convergence properties under appropriate conditions. This rapid convergence is a significant advantage of Newton's method compared to first-order methods such as gradient descent, which typically exhibit only linear convergence.
\else
    これらの\cref{thm:line-search-global-convergence,thm:newton-quadratic}は、ニュートン法が適切な条件の下で大域収束性と局所二次収束性の両方を持つことを示しています。この高速な収束性は、通常は線形収束にとどまる勾配降下法などの一階法と比べたときの大きな利点となります。
\fi
...
\begin{enumerate}[nosep,label=\textbullet]
    ...
    \item $\max_x \lvert f''(x) \rvert = \frac{1}{4} + \mu$ (attained at $e^x=1$). Hence, $\nabla f$ is $L$-smooth with $L=\frac{1}{4}+\mu$.
    \item $\max_x \lvert f'''(x) \rvert = \frac{1}{6\sqrt{3}}$ (attained at $e^x=2-\sqrt{3}$). Hence, $\nabla^2 f$ is $L^\mathrm{H}$-Lipschitz with $L^{\mathrm{H}}=\frac{1}{6\sqrt{3}}$.
          \else
    \item $\mu$-強凸である。
    \item $\max_x \lvert f''(x) \rvert = \frac{1}{4} + \mu$ であり、これは $e^x=1$ で達成される。従って $\nabla f$ は $L$-平滑である ($L=\frac{1}{4}+\mu$)。
    \item $\max_x \lvert f'''(x) \rvert = \frac{1}{6\sqrt{3}}$ であり、これは $e^x=2-\sqrt{3}$ で達成される。従って $\nabla^2 f$ は $L^\mathrm{H}$-リプシッツ連続である ($L^\mathrm{H}=\frac{1}{6\sqrt{3}}$)。
          \fi
\end{enumerate}
\ifEn
