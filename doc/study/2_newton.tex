...
\begin{theorem}[{\cite[Theorem 3.5]{nocedal1999numerical}}]
    ...
    \else
        ...
        すべての $k$ で $\alpha_k=1$ とし、初期点 $x_0$ が $x^*$ に十分近いとき、勾配ノルム列 $\{\norm{\nabla f(x_k)}\}$ は二次収束する。
    \fi
\end{theorem}
\begin{proof}
    \ifEn
    ...
    \else
        $\nabla^2 f$ が定数 $L^{\mathrm{H}}$ を持つ Lipschitz 連続であれば、被積分関数は $L^{\mathrm{H}}t\norm{x_k-x^*}$ で上から抑えられ、積分により次を得る。
    \fi
    \begin{equation*}
        \norm{\nabla^2 f(x_k)(x_k-x^*)-\qty(\nabla f(x_k)-\nabla f(x^*))}
    ...
    \else
        $\nabla^2 f(x^*)$ は非特異なので、ある半径 $r>0$ が存在して $\norm{x_k-x^*}\le r$ を満たすすべての $x_k$ について
        \[
            \norm{\qty(\nabla^2 f(x_k))^{-1}}\le 2\norm{\qty(\nabla^2 f(x^*))^{-1}}
        \]
        が成り立つ。
    ...
    \else
        $\widetilde L  \coloneqq  L^{\mathrm{H}}\norm{\qty(\nabla^2 f(x^*))^{-1}}$ とおく。
        初期点が $\norm{x_0-x^*}\le \min\{r,1/(2\widetilde L)\}$ を満たすように選ばれていれば、帰納法により $\{ x_k \}$ は近傍内に留まり $x^*$ に収束する。
        上の誤差評価は $\{ x_k \}$ の二次収束を示す。

        勾配ノルムの二次収束を示すために、$x_{k+1}-x_k=-\qty(\nabla^2 f(x_k))^{-1}\nabla f(x_k)$ と
        $\nabla f(x_k)+\nabla^2 f(x_k)\qty(x_{k+1}-x_k)=0$ を用いると次を得る。
    \fi
    \begin{align*}
        \norm{\nabla f(x_{k+1})}
...
\begin{figure}[t]
    ...
    \caption{\ifEn An example where Newton's method diverges with initial point $x_0=1.1$\else 初期点 $x_0=1.1$ でニュートン法が発散する例\fi}
