...
\ifEn
    Recall that the SR1 update adds an update term as $z z^\top$. Instead, we can consider an asymmetric rank-one update of the form $z c^\top$ for some vector $c \in \mathbb{R}^n$. We symmetrize the result at the end.
\else
...
\fi
\ifEn
    The initial Hessian approximation $B_0$ is chosen to have eigenvalues 1 and $\lambda_1$, where $\lambda_1$ represents the degree of error in the initial approximation. The initial point $x_0$ is selected as
\else
