...
\else
    この更新則は、対称性を保たないという点に注意して下さい。以下では、その点を改善した代表的な準ニュートン更新則を紹介します。
\fi
...
\else
    なお、このBFGS更新における $H_{k+1}$ の公式は、DFP更新則における $B_{k+1}$ の形式と同じであることに注意して下さい。これらは双対の関係にあります。
...
\else
    ...
    この選択の詳細は~\citep{powellHowBadAre1986}を参照して下さい。
\fi
\ifEn
...
\else
    ...
    この工夫により空間計算量は $\mathcal{O}(nm)$ に減少し、$m$ が小さな定数(通常 $m\le 10$) のとき大幅な改善となります。
