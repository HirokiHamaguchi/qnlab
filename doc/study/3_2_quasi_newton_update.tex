...
\begin{proof}
    \ifEn
        Instead of directly solving the optimization problem, we note that we can think of optimizing the function $\tilde{B} \mapsto \left\lVert \tilde{B} - B \right\rVert_F^2$, which is convex on $\mathbb{R}^{n \times n}$.
        The constraint set
        \begin{equation*}
    ...
    \end{equation*}
    \ifEn
        We used the submultiplicativity of the Frobenius norm and the fact that $\left\lVert ss^\top/(s^\top s) \right\rVert_F=1$.
