...
\begin{proof}
    ...
    \end{align*}
    \ifEn
        This shows the convergence of the even subsequence $\{ C_{2t} \}_{t=0}^{\infty}$ to $C_{\infty}$.

        Next, for the odd subsequence, we have
    \else
        これで偶数番目の部分列 $\{ C_{2t} \}_{t=0}^{\infty}$ が $C_{\infty}$ に収束することが示されました。
    ...
    \end{equation*}
    \ifEn
        Therefore both subsequences $\{ C_{2t} \}_{t=0}^{\infty}$ and $\{ C_{2t+1} \}_{t=0}^{\infty}$ converge to $C_{\infty}$, completing the proof.
    \else
        従って、部分列 $\{ C_{2t} \}_{t=0}^{\infty}$ と $\{ C_{2t+1} \}_{t=0}^{\infty}$ はどちらも $C_{\infty}$ に収束し、証明が完了します。
    \fi
    \myQED
\end{proof}
