...
\ifEn
    Let us derive the corresponding modified secant condition for $B_{k+1}^{\mathrm{F}}$. Substituting the definition of $m^{\mathrm{F}}_{k+1}$ and using $s_k = x_{k+1} - x_k$, the condition becomes
\else
...
\end{equation*}
\ifEn
    Here, $\sigma^\mathrm{F}_k \in \mathbb{R}$ is a scalar to be determined as follows. Substituting this equation yields
\else
...
\begin{figure}[t]
    ...
    \caption{
        \ifEn
            Function-value-matching modified secant equation. The function values $f(x_{k-1})$ and $m^{\mathrm{F}}_k(x_{k-1})$ are matched at the previous point $x_{k-1}$.
        \else
            関数値一致の修正セカント方程式。前の点 $x_{k-1}$ での関数値 $f(x_{k-1})$ と $m^{\mathrm{F}}_k(x_{k-1})$ が一致している。
        \fi
    }
    \label{fig:yuan_msec}
\end{figure}
...
\end{equation*}
\ifEn
    Again, for some scalar $\sigma^\mathrm{C}_k \in \mathbb{R}$, we assume $\left( B^{\mathrm{C}}_{k+1} + \sigma^\mathrm{C}_k I \right) s_k = y_k$. Then, the previous equation yields
\else
