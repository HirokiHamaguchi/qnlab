...
\begin{figure}[t]
    ...
    \caption{
    \ifEn
        Function-value-matching modified secant equation. The function values $f(x_{k})$ and $m_k^{\mathrm{F}}(x_{k})$ are matched at the previous point $x_{k}$.
    \else
        関数値一致の修正セカント方程式。前の点 $x_{k}$ での関数値 $f(x_{k})$ と $m_k^{\mathrm{F}}(x_{k})$ が一致している。
    \fi
    }
    \label{fig:yuan_msec}
\end{figure}
