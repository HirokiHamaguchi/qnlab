...
\ifEn
    First, recall that the standard secant condition used in quasi-Newton methods is given by
\else
    まず、準ニュートン法で使用される標準的なセカント条件は以下の通りでした:
\fi
\begin{equation*}
    B_{k+1} s_k = y_k.
\end{equation*}
...
\else
    ...
    したがって、セカント条件を使用することにより、二次モデル $m_{k+1}(x)$ が前の関数値 $f(x_k)$ を除いて、$x_{k+1}$ と $x_k$ の両方での勾配情報を正確に反映します。
\fi
...
\begin{figure}[t]
    ...
    \caption{
        \ifEn
            Motivation for modified secant equations. The standard secant update omits $f(x_k)$ and can misestimate curvature, while the modified secant condition combines function values with gradients, aiming to approximate the ideal quadratic model.
        \else
            修正セカント方程式の動機。 標準的なセカント更新は $f(x_k)$ を使用せず、曲率を誤推定する可能性がある。関数値と勾配を組み合わせた修正セカント条件は理想的な二次モデルを近似することを目的としています。
        \fi
    }
