...
\else
    ...
    これらの指標は通常、決定変数から実数への写像である目的関数としてモデル化される。
\fi
...
\ifEn
    ...
    The function $f$ is convex and $\mu$-strongly convex if and only if the following hold for any two points $x,y \in \mathbb{R}^n$ and $\lambda \in [0,1]$:
\else
    凸性(convexity)と強凸性(strong convexity)は最適化理論の基本概念である。
    簡単のため、$f$ は $\mathbb{R}^n$ 全域で実数値を取る関数である、つまり、$f \colon \mathbb{R}^n \to \mathbb{R}$ とする。また、$\mu>0$ を定数とする。
    この時、関数 $f$ が凸あるいは $\mu$-強凸であることは、任意の $x,y \in \mathbb{R}^n$ と $\lambda \in [0,1]$ について次が成り立つこととそれぞれ同値である:
\fi
\begin{align*}
    \textrm{(convex)} \quad
...
\else
    ...
    このような定義は、文献によって多少の揺れがあるが、例えば\citep{nesterovIntroductoryLecturesConvex2014,kanamori2016continuous}などを参照されたい。
\fi
...
\ifEn
    There are alternative definitions of convexity and strong convexity using the gradient of $f$.
    Assume that $f$ is at least of class $C^1$ and it takes real values over the domain $\mathbb{R}^n$.
    The function $f$ is convex and $\mu$-strongly convex if and only if the following hold for any $x,y \in \mathbb{R}^n$:
\else
    関数 $f$ の勾配を用いた凸性および強凸性の別の定義も存在する。
    $f$ が少なくとも $C^1$ 級で、その定義域 $\mathbb{R}^n$ 全体で実数値を取ると仮定する。
    関数 $f$ が凸あるいは $\mu$-強凸であることは、任意の $x,y \in \mathbb{R}^n$ について次が成り立つこととそれぞれ同値である:
\fi
\begin{align*}
    \textrm{(convex)} \quad
    f(y) & \ge f(x)+\nabla f(x)^\top (y-x),                                  \\
...
\begin{proposition}
    ...
    \else
        $f$ が $C^1$ 級かつ定義域が $\mathbb{R}^n$ であるとき、
        凸と $\mu$-強凸に関する二つの定義はそれぞれ同値である。
    \fi
\end{proposition}
\begin{proof}
    \ifEn
    ...
    \else
        $\mu$-強凸性の場合について示す。
        まず、前者の定義が成立することを仮定する。勾配の定義と、$f$ が $C^1$ 級であることから、
    \fi
    \begin{align*}
        \nabla f(x)^\top (y-x)
    ...
    \else
        ...
        続いて、後者の定義が成立することを仮定する。$z \coloneqq (1-\lambda)x + \lambda y$ とおくと、
    \fi
    \begin{align*}
    ...
    \else
        これらを、それぞれ $1-\lambda$ 倍、$\lambda$ 倍して足し合わせると、
    \fi
    \begin{align*}
...
\else
    次に、$f$ が $C^2$ 級であるとして、凸性および強凸性がヘッセ行列 $\nabla^2 f(x)$ の正定値性とどのように関係するかを示す。
...
\begin{proof}
    ...
    \end{equation*}
    \ifEn
        where $r(t)$ satisfies $\lVert r(t) \rVert/t \to 0$ as $t \to 0$, and we can rewrite it as
    \else
