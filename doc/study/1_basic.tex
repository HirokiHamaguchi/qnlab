...
\ifEn
    ...
    Assume that $f$ is of class $C^1$ and it takes real values over the domain $\mathbb{R}^n$.
...
\else
    ...
    \cref{fig:convexity_comparison} に示した通り、凸関数と強凸関数には曲率に違いがみられます。
...
\ifEn
    ...
    For $\mu \geq 0$, the condition $A \succeq \mu I$ is equivalent to $v^\top A v \ge \mu \left\lVert v \right\rVert^2$ for all $v \in \mathbb{R}^n$.
    It directly implies that all eigenvalues of $A$ are at least $\mu$, which in turn implies that the operator norm $\left\lVert A \right\rVert$ is at least $\mu$, i.e., $\left\lVert A \right\rVert \geq \mu$.
\else
...
\ifEn
    holds for all $x,y$.
    The $L$-smoothness of $f$ is equivalent to the $L$-Lipschitz continuity of the gradient $\nabla f$ by definition.
    Please also refer to \cref{fig:l-smooth}.
\else
    が任意の $x,y$ について成り立つことと同値です。
    定義そのものですが、関数 $f$ が $L$-平滑であることは、勾配 $\nabla f$ が $L$-リプシッツ連続であることと同値です。
    \Cref{fig:l-smooth}も参照して下さい。
\fi
...
\begin{figure}[htbp]
    ...
    \caption{
        \ifEn
            An example of $L$-smooth function in one-dimensional space.
            $L$-smoothness gives a bound on the variation of the function $f$ (the orange dashed line is an upper bound on $f(x)$), and it is equivalent to the Lipschitz continuity of $\nabla f$ ($\nabla f(x)$ only exists in the gray area).
        \else
            1次元における $L$-平滑関数の例。
            $L$-平滑性は、関数 $f$ の変動幅に有界性を与え(オレンジ色の破線が $f(x)$ の上界)、また $\nabla f$ のリプシッツ連続性と同値(灰色の領域にしか、$\nabla f(x)$ は存在しない)です。
        \fi
    }
    \label{fig:l-smooth}
\end{figure}
...
\begin{proof}
    ...
    \ifEn
        ...
        Moreover, the $L$-smoothness of $f(z)$ directly implies the $L$-smoothness of $g_x(z)$.
        Thus, by the $L$-smoothness of $g_x(z)$, we have
    \else
