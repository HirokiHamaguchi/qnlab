...
\else
    ...
    このような定義は、文献によって多少の揺れがありますが、例えば\citep{nesterovIntroductoryLecturesConvex2014,kanamori2016continuous}などを参照して下さい。
\fi
...
\ifEn
    There are alternative definitions of convexity or strong convexity using the gradient of $f$.
...
\begin{figure}[t]
    ...
    \caption{
        ...
        \else
            二次モデル
            $f(x)=\frac{1}{2}(x - x_k)^\top H (x - x_k) + \nabla f(x_k)^\top (x - x_k) + f(x_k)$ を二次元空間で示したもの。
...
\else
    ...
    ここでは $C^1$ の微分可能性だけを仮定する点に注意して下さい。
\fi
...
\else
    証明は他の文献を参照して下さい~\citep{bauschkeBaillonHaddadTheoremRevisited2009}~\citep[Proposition 12.60]{rockafellarVariationalAnalysis1998}。
\fi
\ifEn
...
\else
    この不等式は準ニュートン法の一種であるBFGS法やL-BFGS法などの更新公式の解析で用いられています。
\fi
