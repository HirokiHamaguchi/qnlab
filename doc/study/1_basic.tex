...
\ifEn
    ...
    Such definitions may vary slightly in the literature. For example, see~\citep{nesterovIntroductoryLecturesConvex2014}.
\else
...
\end{align*}
\ifEn
    A symmetric matrix is indefinite if its quadratic form takes both positive and negative values; equivalently, it is neither positive semi-definite nor negative semi-definite. For symmetric matrices $A,B \in \mathbb{R}^{n \times n}$, the notation $A \succeq B$ indicates that $A-B$ is positive semi-definite. This relation is called the \href{https://en.wikipedia.org/wiki/Loewner_order}{L{\"o}wner order}. The notation $A \succ B$ means that $A-B$ is positive definite. The symbols $\preceq$ and $\prec$ are defined analogously. In particular, if $B$ is the zero matrix, we may simply write $A \succeq 0$. For $\mu \geq 0$, the condition $A \succeq \mu I$ is equivalent to $v^\top A v \ge \mu \left\lVert v \right\rVert^2$ for all $v \in \mathbb{R}^n$. It directly implies that all eigenvalues of $A$ are at least $\mu$. It also implies that the operator norm $\left\lVert A \right\rVert$ is at least $\mu$, since the operator norm of a symmetric matrix is the largest absolute value of its eigenvalues.
\else
    二次形式が正の値と負の値の両方を取る対称行列、すなわち半正定値でも半負定値でもない対称行列は、不定値 (indefinite) と呼ばれます。対称行列 $A,B \in \mathbb{R}^{n \times n}$ に対して、$A \succeq B$ は $A-B$ が半正定値であることを表します。この関係は \href{https://en.wikipedia.org/wiki/Loewner_order}{L{\"o}wner 順序 (L{\"o}wner order)} と呼ばれます。そして $A \succ B$ は、$A-B$ が正定値であることを意味します。同様に、$\preceq$ や $\prec$ も定義されます。特に $B$ が零行列のときは $A \succeq 0$ と書くことがあります。$\mu \geq 0$ に対し、$A \succeq \mu I$ はすべての $v \in \mathbb{R}^n$ について $v^\top A v \ge \mu \left\lVert v \right\rVert^2$ と同値です。これは $A$ の任意の固有値が少なくとも $\mu$ 以上であることを意味します。更に、対称行列ならば演算子ノルム $\left\lVert A \right\rVert$ は固有値の絶対値の最大値であるため、$\left\lVert A \right\rVert$ も少なくとも $\mu$ 以上であることを意味します。
\fi
...
\begin{proof}
    \ifEn
        Firstly, assume that $f$ is $\mu$-strongly convex. For any $x \in \mathbb{R}^n, \ v \in \mathbb{R}^n$ and $t > 0$, letting $y=x \pm tv$ gives
    \else
    ...
    \ifEn
        Conversely, let $\mu>0$ and assume $\nabla^2 f(x)\succeq \mu I$ for all $x \in \mathbb{R}^n$. By Taylor's theorem, for any $x,y \in \mathbb{R}^n$, letting $\phi(t) \coloneqq f(x+t(y-x))$ gives
    \else
        逆に、$\mu>0$ とし、任意の $x \in \mathbb{R}^n$ に対して $\nabla^2 f(x)\succeq \mu I$ が成り立つと仮定します。テイラーの定理より、任意の $x,y \in \mathbb{R}^n$ について $\phi(t) \coloneqq f(x+t(y-x))$ とおくと、次が成り立ちます。
    \fi
    \begin{align*}
        f(y) & =\phi(1)                                                                                        \\
...
\begin{figure}[htbp]
    ...
    \caption{
        \ifEn
            An example of $L$-smooth function in one-dimensional space. $L$-smoothness gives an upper bound of $f(x)$ as shown by the orange dashed line, and it is equivalent to the Lipschitz continuity of $\nabla f$, which means that $\nabla f$ only exists in the gray area.
        \else
...
\begin{proof}
    \ifEn
        By the fundamental theorem of calculus, for any $x,y \in \mathbb{R}^n$, letting $\phi(t) \coloneqq f(x+t(y-x))$ gives
    \else
        微分積分学の基本定理より、任意の $x,y \in \mathbb{R}^n$ について $\phi(t) \coloneqq f(x+t(y-x))$ とおくと、次が成り立ちます。
    \fi
    \begin{align*}
...
\else
    もし、更に $f$ が凸であれば、次の下界も成立します\citep[Proposition 2.3.5]{FanZhongXiuMingLianSokZuiShiHuaarugorizumu2023}。
\fi
\begin{proposition}
    \ifEn
...
\ifEn
    As we have seen so far, $L$-smoothness means that the rate of gradient change is bounded from above, and this can also be related to the definiteness of the Hessian matrix. In other words, as shown in the following proposition, the Hessian matrix is bounded by $-L I$ and $+L I$.
\else
...
\begin{proposition}\label{prop:smoothness-hessian}
    \ifEn
        Let $f \colon \mathbb{R}^n \to \mathbb{R}$ be of class $C^2$. Then $f$ is $L$-smooth if and only if $-L I \preceq \nabla^2 f(x)\preceq +L I$ holds for all $x \in \mathbb{R}^n$.
    \else
...
\begin{proof}
    ...
    \ifEn
        ...
        In the same manner, $\nabla^2 f(x)\succeq -L I$ can also be shown, which completes the proof.
    \else
...
\fi
\ifEn
    Assume that the function $f$ is convex and $L$-smooth. For the sequences $\{ x_k \}_k$ generated by an optimization algorithm, defining
\else
    関数 $f$ が凸かつ $L$-平滑であるとします。最適化アルゴリズムが生成する列 $\{ x_k \}_k$ に対し、
\fi
\begin{equation*}
