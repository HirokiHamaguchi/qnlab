...
Our numerical experiments in \cref{sec:experiments} include cases where gradients are contaminated by noise, and they also demonstrates that the proposed method remains effective even when \cref{eq:exact_gradient_assumption} is violated.
...
\end{lemma}
\begin{proof}
    Since $-g_k^\top d_k \geq 0$ from \cref{ass:BBounds} and \cref{eq:BBounds_results_zero}, \cref{eq:relaxed_Armijo_condition} implies
    \begin{equation}\label{eq:overline_delta_k_lower_bound}
        \overline{f}(x_k) - \overline{f}(x_{k+1})
        \geq - c \alpha_k g_k^\top d_k -\Delta_k
        \geq -\Delta_k.
    \end{equation}
    Thus, using the general result in \cref{eq:delta_k_error_bound}, we can further bound as
    \begin{align*}
...
\end{equation}
The following summarizes the behavior of the backtracking line search.
...
In this subsection, we bound the sum of actual function decreases $\delta_k$ over $k \in K^0$.
Recall that $K^0$ denotes the set of iteration indices $k$ such that $\mu_k = 0$, as defined in \cref{eq:def_K0_Kplus}.

To this end, we establish that the sum of $\Delta_k$ over $k \in K^0$ is bounded, where $\Delta_k$ is the error-absorbing term in the relaxed Armijo condition (\cref{eq:relaxed_Armijo_condition}).
...
\begin{proof}
    ...
    \end{equation}
    We also have $\overline{f}(x_0) \ge \overline{f}(x_{k_{\ell}})$ from \cref{eq:mu_0_condition} with $k=k_{\ell}$, or just $x_0 = x_{k_{\ell}}$.
    Thus, by \cref{ass:bounded}, we have $\Delta_{k_{\ell}} = \frac{2\feps}{1-\feps} \max(1, \overline{f}(x_{k_{\ell}}), -\overline{f}(x_{k_{\ell}+1})) \le \Delta_{\max}$.
    Hence, we have
    \begin{align*}
        \sum_{k \in K^0} \Delta_{k}
         ...
         & \leq \overline{f}(x_0) - \overline{f}^* + \Delta_{\max}.                                        &  & (\overline{f}(x_{k_{\ell}}) \ge \overline{f}^* \text{ and } \Delta_{k_{\ell}} \le \Delta_{\max})
    \end{align*}
    This completes the proof.
    \myQED
\end{proof}
...
Next, we show that the sum of $\delta_{k}$ over $k \in K^+$ is bounded.
...
Before proceeding to the main proof, we present \cref{lem:sum_norm_g_over_mu_geq_2sqrt_leq_log}, which is standard inequalities in the AdaGrad-Norm algorithm analysis~\citep[Lemma~3.2]{wardAdaGradStepsizesSharp2020},~\citep[Lemma~3.1]{Gratton08022024}.
...
We now bound the sum of $\delta_k$ over $k \in K^+$ with $\norm{g_{k}}^2$.
...
\begin{proof}
    ...
    \begin{align}
        \frac{1}{k_{\max}} \sum_{k=0}^{k_{\max}-1} \norm{g_k}^2
         ...
         & \leq \frac{1}{k_{\max}} \max\qty(\frac{M}{c \overline{\alpha}} F', \, \frac{4}{C_1^2} F'^2), \label{eq:gc_result}
    \end{align}
...
We first discuss the construction of the matrices $B_k$ used in \refLine{line:d_k} of \cref{alg:regularized_qn}, using the L-BFGS method.
The purpose of this subsection is to show that $B_k$ can satisfy \cref{ass:BBounds}, the uniform spectral bound, with a slight modification and selection of curvature pairs.
...
Following the previous work~\citep{kanzowRegularizationLimitedMemory2023}, we excluded problems such as the initial point is already stationary or contains \texttt{NaN} and \texttt{INF} in its function values or gradient.
...
As a side note, although the (\texttt{SciPy}) calls optimized C routines, it incurs associated dispatch overhead, producing a larger mean time. One of the reasons of the (\texttt{Reg}) and (\texttt{Reg-Sec}) methods taking longer execution time is due to the implementation aspect of the original code.
