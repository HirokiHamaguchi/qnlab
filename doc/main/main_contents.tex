...
All primary experiments disable this heuristic and therefore coincide with the analyzed accumulator, while the appendix ablation in \cref{app:restart_experiment} permits at most ten restarts.
\end{MPCchange}
...
\begin{itemize}
    ...
    \MPCchanged{\item (\texttt{ASTR1-Adagrad}): The purely first-order \texttt{adagrad} variant of ASTR1 with the parameter setting reported by Gratton et al.~\citep[Table~1]{Gratton08022024}.
    Its steps and stopping decision use only gradients.
    The experimental logging code records the initial objective value solely for diagnostics and does not pass it to the method.}
\end{itemize}

\begin{MPCchange}
For the primary NTQN comparison, the experimental driver stops NTQN when it attains the same gradient tolerance used to score every method.
We additionally use the authors' recommended termination mode (the option \texttt{terminate=3}) with this external stop disabled.
...
In the recommended mode, NTQN can stop upon attaining its gradient tolerance, reaching an estimated function- or gradient-noise level, or detecting stagnation.
The common gradient tolerance is applied only afterward to its recorded trajectory for uniform scoring.
...
\begin{MPCchange}
We consider the following five primary experimental settings:
\begin{itemize}
    \item \emph{Function-value noise}: 64-bit arithmetic on 220 problems, with $\overline f(x) = f(x) + \xi_f$, $\xi_f \sim \operatorname{unif}(-10^{-3}, 10^{-3})$, exact gradients, and the nominal allowance $\feps = 10^{-2}$.
    \item \emph{Joint function-and-gradient noise}: 64-bit arithmetic on the same 220 problems, with the preceding function perturbation, independent componentwise gradient perturbations $\xi_g^{(i)} \sim \operatorname{unif}(-10^{-3}, 10^{-3})$, and $\feps = 10^{-2}$.
...
\end{itemize}
Here, $\operatorname{unif}(a,b)$ denotes the uniform distribution on $[a,b]$, and all function and gradient perturbations are sampled independently.
The retained problems have dimensions from $2$ to $123{,}200$ in every collection.

For the reduced-precision settings, the input vector is rounded to the target precision before each CUTEst evaluation, and the returned function value and gradient are rounded to the same precision.
CUTEst performs its internal calculations in 64-bit arithmetic, so these experiments simulate the effect of lower-precision inputs and oracle outputs rather than fully lower-precision internal evaluation.
...
To examine misspecification of the error allowance, the function-value-noise experiment additionally uses $\feps=10^{-4}$ and $10^{-1}$.
...
Each additive-noise setting uses the five seeds $0, 1, 2, 3, 4$, with the same seed supplied to every solver for a given problem and scenario.
...
The shifted-pair variant \texttt{Ours-SP} solves $83.9\%$, while the compact solver never uses its zero-memory fallback direction for \texttt{Ours} and uses it nine times in total for \texttt{Ours-MS} across these instances.
...
The corresponding \texttt{Ours-SP} rate is $80.6\%$, and the compact solver uses its zero-memory fallback direction twice for \texttt{Ours} and once for \texttt{Ours-MS}.
...
We demonstrated the method's effectiveness through numerical experiments on standard benchmark problems under various noise levels and precision settings.
