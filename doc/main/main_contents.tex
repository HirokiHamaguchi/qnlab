...
By contrast, choosing a larger value preserves the analysis as long as $\feps<1$ and the error bound remains valid, although it moves the method earlier or more frequently into the more conservative OFFO-type regime $K^+$.
...
We next specify the assumption on gradient evaluations.
...
The proposed method does not use Wolfe conditions and therefore does not require gradient evaluations at trial points to verify such conditions.
...
Before proceeding to the main proof, we present \cref{lem:sum_norm_g_over_mu_geq_2sqrt_leq_log}, \MPCchanged{which collects standard inequalities from the AdaGrad-Norm analysis of Ward et al.~\citep[Lemma~3.2]{wardAdaGradStepsizesSharp2020} and Gratton et al.~\citep[Lemma~3.1]{Gratton08022024}}.
...
Thus the order symbol suppresses no dependence on the iteration counter, but it does suppress these problem and algorithm parameters.
...
\begin{proposition}\label{prop:bounded_BFGS}
    ...
    \end{equation}
\end{proposition}
\MPCchanged{Proposition~\ref{prop:bounded_BFGS} gives the upper spectral bound needed below.}
\MPCchanged{A fixed diagonal shift provides the lower bound in \cref{ass:BBounds}.}

\begin{MPCchange}
We select curvature pairs using a cautious update in the spirit of curvature-based cautious updates~\citep{liGlobalConvergenceBFGS2001,mannelStructuredLBFGSMethod2024,mannelGlobalizationLBFGSBarzilai2025a}.
...
This is the damping rule used in the released code.
The fixed initial-scale upper test and the diagonal shift, rather than damping itself, guarantee the two bounds in \cref{ass:BBounds}.
...
In particular, the implementation computes $d_k=-(B_k+\mu_k I)^{-1}g_k$ for the matrix $B_k$ defined in \cref{eq:proposed_choice_of_Bk}, without replacing the additive diagonal regularization by modified curvature pairs.
We adopted this exact compact solve to align the implemented direction directly with the direction analyzed in \cref{sec:analysis}.
...
\MPCchanged{For the proposed \texttt{-MS} variants, a scale-aware safeguard sets the function-based correction $\sigma_i$ to zero whenever $\abs{\sigma_i}>\sqrt{\norm{y_{k_i}}^2/\norm{s_{k_i}}^2}$, preventing the correction from dominating the raw gradient difference.}
...
Because this optional rule is heuristic, all primary experiments use the analyzed no-restart method.
The variant with at most ten restarts is reserved for the separate appendix ablation in \cref{app:restart_experiment}.
\end{MPCchange}
...
\begin{itemize}
    \item (\texttt{Ours}): Our proposed regularized quasi-Newton method
          \begin{itemize}
              ...
              \MPCchanged{\item The suffix \texttt{-SP} denotes the corresponding heuristic variant that replaces the exact compact solve by the shifted-pair approximation in \cref{eq:approx_computation_of_regularized_quasi_Newton}.}
          \end{itemize}
    ...
    \item (\texttt{NTQN}): Noise-tolerant quasi-Newton method~\MPCchanged{\citep{xieAnalysisBFGSMethod2020,shiNoiseTolerantQuasiNewtonAlgorithm2022}}
          \begin{itemize}
              ...
              \begin{MPCchange}
              \item The primary comparison uses the wrapper's common gradient-tolerance stopping rule.
              \item We also run the authors' recommended termination mode (\texttt{terminate=3}) with the wrapper's gradient-tolerance stop disabled.
              \item In the function-noise experiment, the additive perturbation is bounded by $10^{-3}$, and NTQN receives the conservative function-noise bound $\texttt{eps\_f}=10^{-2}$ used numerically as $\feps$ by the proposed method.
              \item NTQN uses this supplied bound in both its noise-tolerant line search and its recommended termination test.
              \item For the latter, the external solver controls termination.
              \item The common gradient tolerance is used only to score its recorded trajectory and is not injected into the solver.
              \item The recommended mode may appear worse under this metric because it can terminate appropriately at its estimated noise neighborhood before reaching the common scoring tolerance.
              \item This reflects a difference in stopping objectives rather than necessarily worse iterates.
    ...
    \MPCchanged{\item (\texttt{ASTR1-Adagrad}): The purely first-order \texttt{adagrad} variant of ASTR1 reported by Gratton et al.~\citep[Table~1]{Gratton08022024}, using $B_k = 0$, $\gamma_k = 1$, $\mu = 1/2$, $\varsigma = 1/100$, and $\vartheta = \theta = 1$.
    Its steps and stopping decision use only gradients.
    The shared callback records the initial objective value solely for diagnostics and does not expose it to the method.}
\end{itemize}
...
The 64-bit value equals SciPy's default heuristic $10^7\epsilon_{\mathrm{mach}}$.
At lower precisions we reduce the multiplier to $10^4$ and $10^2$, respectively, because retaining $10^7$ would violate the required range $\feps<1$ and yield an uninformative error allowance.
...
Overestimating $\feps$ preserves the error model and hence is safe from the viewpoint of the convergence theory, provided that $\feps<1$.
It relaxes Armijo acceptance through a larger $\Delta_k$, but it also lowers every reference value $\overline f(x_j)-\Delta_j$ and thereby makes subsequent entry into $K^0$ harder.
The latter effect can keep the method in $K^+$, where the nondecreasing scale $G_k$ produces more conservative OFFO-type steps.
Thus overestimation is expected to preserve stability but can reduce practical efficiency, whereas underestimation can place the computation outside the assumed error model.
The sensitivity experiment examines this distinction.
...
The joint-noise setting deliberately violates \cref{eq:exact_gradient_assumption}.
It is included to test performance under an unfavorable perturbation, not to validate the exact-gradient theorem.
...
\MPCchanged{If it attains the scenario's gradient tolerance after $k_{\mathrm{SciPy}}$ iterations, every other method is limited to $\min\{15{,}000,15k_{\mathrm{SciPy}}\}$ iterations.}
\MPCchanged{Otherwise, the limit is $15{,}000$.}
...
The former tests the assumptions of \cref{sec:analysis}.
The latter asks whether the implementation remains useful when both available quantities are perturbed.
...
All main-text profiles use the analyzed no-restart method.
The earlier accumulator-restart heuristic is evaluated separately in the appendix.
Across the $220$ problems and five paired seeds, \texttt{Ours} and \texttt{Ours-MS} attain \(\gtol = 10^{-2}\) on $83.9\%$ and $84.8\%$, respectively, of the $1{,}100$ function-noise instances.
Their seed-to-seed standard deviations in the solved fraction are only $0.52$ and $0.83$ percentage points.
The corresponding rates are $81.2\%$ for \texttt{NTQN} and $77.8\%$ for \texttt{Reg}.
They are $74.0\%$ for \texttt{Reg-Sec} and $41.5\%$ for \texttt{ASTR1-Adagrad}.
Among successful runs, the median oracle-call counts are $62$ for \texttt{Ours}, $71$ for \texttt{Ours-MS}, $64$ for \texttt{NTQN}, and $131$ for \texttt{ASTR1-Adagrad}.
The complete distributions, including failures, are represented by the profiles in \cref{fig:results_function_noise}.
Both proposed variants solve at least one instance of dimension $123{,}200$, the largest dimension in the retained collection.
In the joint-noise stress test, \texttt{Ours} and \texttt{Ours-MS} solve $80.7\%$ and $83.1\%$ of the instances, compared with $71.9\%$ for \texttt{NTQN}, $73.5\%$ for \texttt{Reg}, and $42.8\%$ for \texttt{ASTR1-Adagrad}.
The two proposed rates have seed-to-seed standard deviations of $0.61$ and $0.75$ percentage points.
...
Under NTQN's recommended termination mode, $50.1\%$ of the same function-noise instances attain the common tolerance, compared with $81.2\%$ under the wrapper's common-tolerance stop.
All recommended-mode runs terminate without timeout or exception, and $619$ of them return flag~4.
...
The resulting reference shift keeps the method in $K^+$ more often and produces conservative steps through the accumulated regularization scale.
Although this behavior does not invalidate the theory, the results show that a safety margin that is much larger than the actual error can reduce practical efficiency.
By contrast, underestimation can violate the assumed error bound even though it happens to improve the aggregate success rate of the unmodified variant in this experiment.
This asymmetric response supports treating \(\feps\) as a practically consequential error allowance rather than as a parameter for which either direction of misspecification is uniformly advantageous.
