...
\MPCchanged{We now give a sufficient condition for the upper spectral bound in \cref{ass:BBounds}, based on variants of existing results~\citep[Theorem~5.5]{gaoQuasiNewtonMethodsSuperlinear2018} \citep[Lemma~1]{gowerStochasticBlockBFGS2016}.} % to LLM: do not insert `and' between the two citations.
...
The numerical experiments distinguish the no-restart and restarted variants.
The former coincides with the analyzed accumulator, whereas the latter uses $\tau=1$ and at most $10$ restarts.
\end{MPCchange}
...
\begin{itemize}
    \item (\texttt{Ours}): Our proposed regularized quasi-Newton method
          \begin{itemize}
              ...
              \MPCchanged{\item (\texttt{Ours-R}) means the heuristic variant incorporating the optional accumulator restart described in \cref{sec:proposed_choice_mu}.}
          \end{itemize}
...
In the theory-aligned function-value-noise setting, \texttt{Ours-R} solves $87.6\%$ of the instances, compared with $85.1\%$ for \texttt{Ours}, and reduces the conditional median from $62$ to $60$ oracle calls.
...
\begin{MPCchange}
\Cref{fig:results_error_bound_sensitivity} shows the performance profiles for the error-bound sensitivity experiment.
...
The optional restart curves permit a direct comparison with this heuristic.
\end{MPCchange}
