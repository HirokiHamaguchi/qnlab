...
Notably, the proposed algorithm does not rely on Wolfe-type conditions, which typically require accurate gradient information at trial points, and is therefore robust to gradient inaccuracies.
...
\end{equation*}
which is consistent with the general form stated in \cref{eq:offo_based_update} with an adaptive factor $\theta_k \in [10^{-2},1]$.
...
When the difference between the left- and right-hand sides of \cref{eq:mu_0_condition} is larger than $1$, we restart the accumulation by resetting $K^+$ to the empty set and $G_k$ to $\varsigma$.
...
\begin{itemize}
    ...
    \item 32-bit floating-point precision (220 problems, $\feps=1.19 \times 10^{-3}$)
    \item 16-bit floating-point precision (152 problems, $\feps=9.77 \times 10^{-2}$)
\end{itemize}
...
We use the number of oracle calls rather than clock time to avoid the influence of algorithmic overhead and to focus on the intrinsic optimization efficiency.
