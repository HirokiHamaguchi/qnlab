...
\begin{MPCchange}
Settings in which objective values are accurate but gradients are noisy, such as derivative-free optimization or stochastic approximation methods~\citep{berahasTheoreticalEmpiricalComparison2021,xieAnalysisBFGSMethod2020,shiNoiseTolerantQuasiNewtonAlgorithm2022}, are beyond the scope of this work.
\end{MPCchange}
...
\begin{algorithm}[ht]
    ...
    \For{$k=0,1,2,\ldots, k_{\max}-1$}{
        ...
        $g_{k+1} \gets g(x_{k+1})$ \label{line:update}\;
    }
...
\begin{MPCchange}
In addition to the exact-gradient assumption stated in \cref{ass:exact_gradient}, we make the following three structural assumptions.
\end{MPCchange}
...
\begin{proof}
    ...
    \end{equation*}
    Since $-g_k^\top d_k \geq 0$ from \cref{ass:BBounds} and \cref{eq:BBounds_results_zero}, the condition of \cref{lem:error_bound_on_function_evaluations} is satisfied with $x_{k+1} = x_k + \alpha_k d_k$. Thus we obtain $\delta_k \le \overline{\delta}_k + \Delta_k$, i.e.,
    \begin{equation*}
...
We introduce the following constant $\Delta_{\max}$, which serves as an upper bound for $\Delta_k$ of $k \in K^0$.
\begin{equation*}
...
\begin{MPCchange}
Recall that $K^+$ is the set of iteration indices $k$ such that $\mu_k > 0$ as defined in \cref{eq:def_K0_Kplus}, and that $\mu_k$ is defined as $\theta_k \sqrt{\varsigma + \sum_{j \in K^+,\ j \leq k} \norm{g_{j}}^2}$ with $\theta_k \in [\theta_{\min}, \theta_{\max}]$ in \refLine{line:update_theta} of \cref{alg:regularized_qn}.
\end{MPCchange}
...
\begin{theorem}\label{thm:gc}
    \begin{MPCchange}
        Under \cref{ass:exact_gradient,ass:bounded,ass:LSmooth,ass:BBounds}, the average of squared gradient norms generated by \cref{alg:regularized_qn} satisfies
    \end{MPCchange}
    \begin{equation*}
...
Finally, \cref{thm:gc} implies that the iteration complexity:
\begin{equation*}
...
\end{equation*}
Here \(\overline\alpha\) depends only on \(c,L,m,M\) and the backtracking bounds, \(C_1=\overline\alpha/\theta_{\max}\), \(C_2=(M\overline\alpha+L/2)/\theta_{\min}^2\), and \(F'\) is displayed in the proof and contains \(\feps\), \(\varsigma\), the initial true and observed optimality gaps, and \(\Delta_{\max}\).
...
\begin{MPCchange}
We briefly note the L-BFGS method and the curvature pair notation~\citep{nocedal1999numerical,liuLimitedMemoryBFGS1989,berahasLimitedmemoryBFGSDisplacement2022}.
\end{MPCchange}
...
If no curvature pair is stored, including when all candidate pairs have been rejected, the implementation uses $B_k = \gamma_0 I$.
Define $\gamma_0$ by
\begin{equation*}
    \gamma_0 = \begin{cases}
        \norm{g_0}, & \norm{g_0} > 0,\\
    ...
    \end{cases}
\end{equation*}
Thus the zero-memory case is also positive definite and has iteration-independent spectral bounds.
...
\begin{MPCchange}
When $\mu_k=0$, the direction can be computed efficiently by the standard L-BFGS two-loop recursion~\citep{liuLimitedMemoryBFGS1989,nocedal1999numerical}.
\end{MPCchange}
...
\begin{MPCchange}
In our implementation, the OFFO accumulator contains exactly those gradient norms generated during regularized iterations:
\begin{equation*}
...
The restart was originally intended only as a practical heuristic used a small finite number of times after unusually large certified progress, rather than as an essential part of the proposed algorithm.
More precisely, for a fixed threshold $\tau>0$, the optional variant resets only the accumulator in \cref{eq:offo_based_update} whenever
...
As proved in \cref{prop:finite_accumulator_restarts}, the positive fixed threshold itself gives a finite problem-dependent upper bound on the number of restarts.
Applying the accumulator argument separately between consecutive restarts then preserves the convergence rate of \cref{thm:gc}, as stated in \cref{thm:gc_with_accumulator_restarts}.
For the restart ablation, we additionally impose the safeguard $R=10$, independent of the iteration limit, and record the actual number of restarts in every run.
The experiments will report the restart-count distribution and whether the cap is ever reached, which directly tests whether repeated restart is confined to pathological cases.
\end{MPCchange}
...
\begin{itemize}
    ...
    \begin{MPCchange}
    \item (\texttt{NTQN}): Noise-tolerant quasi-Newton method~\citep{xieAnalysisBFGSMethod2020,shiNoiseTolerantQuasiNewtonAlgorithm2022}
    \end{MPCchange}
          \begin{itemize}
...
The experiment notebook also compares the hybrid method with an always-OFFO variant and the earlier accumulator-restart heuristic, and varies the supplied \(\feps\) over two orders of magnitude around its nominal value.
For the restart ablation, we aggregate the number of restarts over problems and seeds, report its distribution and maximum, and state whether the fixed cap $R=10$ is reached.
\end{MPCchange}
...
\begin{figure}[t]
    ...
    \end{subfigure}
    \begin{MPCchange}
        \caption{Performance profile for the deliberately adverse joint function-and-gradient noise setting (componentwise level \(10^{-3}\)) and \(\gtol=10^{-2}\). This experiment lies outside the exact-gradient theory.}
    \end{MPCchange}
...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{0.8\textwidth}
        ...
        \includegraphics[width=\textwidth]{../imgs/compare/_legend.pdf}
    \end{subfigure}
    \caption{Performance profiles across different floating-point precisions (64-, 32-, and 16-bit) and tolerance levels. The results demonstrate the robustness of the proposed methods.}
...
In this subsection, we present an experiment to compare the per-iteration computational cost of each method. Here, our purpose lies in the measurement of the total execution time rather than the number of oracle calls.
...
Wall-clock comparisons require particular care because they combine mathematical differences between algorithms with implementation and machine effects.
Although \texttt{SciPy} uses a compiled native implementation, our implementation has also been engineered to use Basic Linear Algebra Subprograms (BLAS), column-contiguous L-BFGS storage, and a ring buffer that avoids shifting the stored curvature pairs.
In a separate diagnostic benchmark, the relative timing changed sharply between $n=10{,}000$ and $n=11{,}000$ with the default BLAS thread pool, whereas this crossover disappeared when BLAS was restricted to a single thread.
This behavior indicates that BLAS thread-dispatch and memory-management details can dominate the runtime ranking even when the mathematical algorithms are unchanged.
The supplied research implementations of \texttt{Reg}, \texttt{Reg-Sec}, and \texttt{NTQN} have not been engineered using the same implementation choices, so additional wall-clock comparisons would likely favor our code partly for reasons unrelated to the algorithms themselves.
We therefore avoid interpreting such an advantage as a mathematical one and place greater weight on oracle-call and success-profile comparisons.
Moreover, absolute runtimes and sometimes their ranking depend on the processor, BLAS library and threading configuration, operating system, and software environment, which limits their reproducibility across machines.
...
Under exact gradients and explicit uniform spectral bounds on the quasi-Newton matrices, the method has the standard \(\order{1/\varepsilon^2}\) first-order iteration complexity.
