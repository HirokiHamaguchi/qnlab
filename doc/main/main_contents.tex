...
Now, we establish the following \MPCchanged{iteration-complexity result} for \cref{alg:regularized_qn}.
\begin{theorem}\label{thm:gc}
    \MPCchanged{Under \cref{ass:exact_gradient,ass:bounded,ass:LSmooth,ass:BBounds}, for every $0<\varepsilon\leq1$, \cref{alg:regularized_qn} generates an iterate satisfying $\norm{g_k}\leq\varepsilon$ within $\order{1/\varepsilon^2}$ iterations.}
\end{theorem}
\begin{proof}
    ...
    \begin{MPCchange}
    Suppose that $\norm{g_k}>\varepsilon$ for every $0\leq k<k_{\max}$, where $0<\varepsilon\leq1$.
    We then have $\sum_{k\in K^0}\norm{g_k}^2>\varepsilon^2\abs{K^0}$.
    We also have $\sqrt{\sum_{k\in K^+}\norm{g_k}^2}>\varepsilon\sqrt{\abs{K^+}}$.
    Applying these two bounds to \cref{eq:final_inequality_for_gc} yields
    \begin{equation*}
        \abs{K^0}<\frac{MF'}{c\overline\alpha\varepsilon^2},
        \qquad
        \abs{K^+}<\frac{4(F')^2}{C_1^2\varepsilon^2}.
    \end{equation*}
    Since $k_{\max}=\abs{K^0}+\abs{K^+}$, there must therefore be an iterate with $\norm{g_k}\leq\varepsilon$ whenever
    \begin{equation}\label{eq:gc_result}
        k_{\max}\geq\frac{1}{\varepsilon^2}\qty(\frac{MF'}{c\overline\alpha}+\frac{4(F')^2}{C_1^2}).
    \end{equation}
    This completes the proof.
    \end{MPCchange}
    \myQED
...
\begin{equation*}
    ...
    \qty(\frac{MF'}{c\overline\alpha}+\frac{4(F')^2}{C_1^2}).
\end{equation*}
...
\begin{proposition}\label{prop:bounded_BFGS}
    ...
    \MPCchanged{Assume that there exists a constant $\Lambda>0$ such that every pair satisfies}
    \begin{equation}
    ...
    \end{equation}
\end{proposition}
\MPCchanged{Proposition~\ref{prop:bounded_BFGS} gives the upper spectral bound needed below; a fixed diagonal shift will provide the lower bound in \cref{ass:BBounds}.}

\begin{MPCchange}
The original manuscript already intended the pair selection as a cautious update in the spirit of curvature-based cautious updates~\citep{liGlobalConvergenceBFGS2001,mannelStructuredLBFGSMethod2024,mannelGlobalizationLBFGSBarzilai2025a}.
...
\end{equation*}
After damping, a raw pair \((s,\overline y)\) is retained only if
\begin{equation}\label{eq:implemented_cautious_test}
    \overline y^\top s \geq \max\left\{\frac{\norm{\overline y}^2}{10^8\rho_0},10^{-16}\right\}.
\end{equation}
For the proof only, define $\widehat y\defeq\overline y/\rho_0$.
Then every stored pair $(s,\widehat y)$ satisfies \cref{eq:curvature_conditions1} with the iteration-independent constant $\Lambda=10^8$.
The absolute floor rejects numerically degenerate pairs but is not needed for the upper bound.
The homogeneity of the BFGS update gives
\begin{equation*}
    \mathrm{BFGS}(\{(s_{k_i},\overline y_{k_i})\})
    =\rho_0\mathrm{BFGS}(\{(s_{k_i},\widehat y_{k_i})\}).
\end{equation*}
Using at most $p=10$ pairs, we set
\begin{equation}\label{eq:proposed_choice_of_Bk}
    ...
    \end{cases}
\end{equation}
By \cref{prop:bounded_BFGS}, the homogeneity identity, and $\omega_{\min}\leq1\leq\rho_0$, it follows that
\begin{equation*}
...
Candidates for which this scalar damping is numerically unsafe are discarded before \cref{eq:implemented_cautious_test}.
This is the damping rule used in the released code; the fixed initial-scale upper test and the diagonal shift, rather than damping itself, guarantee the two bounds in \cref{ass:BBounds}.
No convexity or cocoercivity assumption on \(\nabla f\) is used.
\end{MPCchange}
...
\MPCchanged{Regularized limited-memory schemes may be employed to account for the diagonal shift~\citep{kanzowRegularizationLimitedMemory2023}.}
\MPCchanged{We use the simple approximation mentioned in the same reference,}
\begin{MPCchange}
...
\end{equation}
\end{MPCchange}
\MPCchanged{The direction is then computed from the raw curvature pairs using the standard two-loop recursion~\citep{liuLimitedMemoryBFGS1989,nocedal1999numerical}; when the memory is empty, the implementation uses $d_k=-g_k/(\rho_0+\omega_{\min}+\mu_k)$.}
...
For the optional heuristic, a fixed threshold $\tau>0$ resets only the accumulator in \cref{eq:offo_based_update} when the method returns from $K^+$ to $K^0$ and
...
\begin{itemize}
    ...
    \MPCchanged{\item (\texttt{ASTR1-Adagrad}): The purely first-order \texttt{adagrad} variant of ASTR1 reported by Gratton et al.~\citep[Table~1]{Gratton08022024}, using $B_k = 0$, $\gamma_k = 1$, $\mu = 1/2$, $\varsigma = 1/100$, and $\vartheta = \theta = 1$. Its steps and stopping decision use only gradients; the shared callback records the initial objective value solely for diagnostics and does not expose it to the method.}
\end{itemize}
...
Overestimating $\feps$ has two competing effects: it relaxes Armijo acceptance through a larger $\Delta_k$, but it lowers every reference value $\overline f(x_j)-\Delta_j$ and thereby makes subsequent entry into $K^0$ harder.
The latter effect can keep the method in $K^+$, where the accumulated scale $G_k$ is nondecreasing and can produce increasingly damped steps; meanwhile, the relaxed Armijo test may accept steps on the basis of an allowance far larger than the actual $10^{-3}$ perturbation.
...
\MPCchanged{We first run \texttt{SciPy} with a limit of $15{,}000$ iterations on each problem.}
\MPCchanged{If it attains the scenario's gradient tolerance after $k_{\mathrm{SciPy}}$ iterations, every other method is limited to $\min\{15{,}000,15k_{\mathrm{SciPy}}\}$ iterations; otherwise, the limit is $15{,}000$.}
We timed out runs exceeding 10 minutes per problem.
...
\MPCchanged{All reported success percentages use the same common gradient tolerance as the corresponding profiles; a run that terminates without attaining that tolerance is counted as unsolved for this metric.}
...
The experiment notebook compares the hybrid method with the first-order \texttt{ASTR1-Adagrad} baseline of Gratton et al.~\citep{Gratton08022024} and varies the supplied \(\feps\) over two orders of magnitude around its nominal value.
...
Across the $220$ problems and five paired seeds, \texttt{Ours} and \texttt{Ours-MS} attain \(\gtol = 10^{-2}\) on $83.9\%$ and $84.8\%$, respectively, of the $1{,}100$ function-noise instances; their seed-to-seed standard deviations in the solved fraction are only $0.52$ and $0.83$ percentage points.
The corresponding rates for \texttt{NTQN}, \texttt{Reg}, \texttt{Reg-Sec}, and \texttt{ASTR1-Adagrad} are $81.2\%$, $77.8\%$, $74.0\%$, and $41.5\%$.
Among successful runs, the median oracle-call counts are $62$ for \texttt{Ours}, $71$ for \texttt{Ours-MS}, $64$ for \texttt{NTQN}, and $131$ for \texttt{ASTR1-Adagrad}; the complete distributions, including failures, are represented by the profiles in \cref{fig:results_function_noise}.
Both proposed variants solve at least one instance of dimension $123{,}200$, the largest dimension in the retained collection.
In the joint-noise stress test, \texttt{Ours} and \texttt{Ours-MS} solve $80.7\%$ and $83.1\%$ of the instances, compared with $71.9\%$ for \texttt{NTQN}, $73.5\%$ for \texttt{Reg}, and $42.8\%$ for \texttt{ASTR1-Adagrad}; the two proposed rates have seed-to-seed standard deviations of $0.61$ and $0.75$ percentage points.
Here the componentwise gradient perturbation $10^{-3}$ is one order below the common scoring tolerance, and the regularized $K^+$ steps damp the effect of an inaccurate gradient when function comparisons cease to certify progress.
This offers a plausible explanation for the strong empirical result, including the higher solved fraction than NTQN under the common stopping criterion, but it does not establish gradient-noise tolerance or extend the exact-gradient theorem.
\end{MPCchange}
...
\begin{figure}[t]
    ...
    \end{subfigure}
    \caption{\MPCchanged{Theory-aligned function-value-noise results at noise level $10^{-3}$, supplied error bound \(\feps = 10^{-2}\), and \(\gtol = 10^{-2}\).}}
...
\begin{MPCchange}
For the error-bound sensitivity study, underestimating \(\feps\) as $10^{-4}$ gives solved fractions of $85.5\%$ for \texttt{Ours} and $75.3\%$ for \texttt{Ours-MS}, whereas the nominal $10^{-2}$ gives $83.9\%$ and $84.8\%$, and overestimating it as $10^{-1}$ gives $72.5\%$ and $71.5\%$.
Thus, underestimation happens to improve the aggregate success rate of the unmodified variant but substantially harms the modified-secant variant, while the tenfold overestimate reduces both rates by about $12$--$13$ percentage points relative to the nominal setting.
For $\feps=10^{-1}$, the multiplier $2\feps/(1-\feps)$ in $\Delta_k$ equals approximately $0.222$, more than one hundred times the actual additive-noise level.
The resulting reference shift can force prolonged $K^+$ use and an overly large accumulated regularization scale, while the simultaneously loosened Armijo test no longer discriminates well between genuine decrease and an allowed increase.
This explains why overestimation is not simply ``safer'': both parts of the hybrid decision become poorly calibrated, yielding conservative steps without a commensurate gain in noise protection.
This asymmetric response supports treating \(\feps\) as a practically consequential error bound rather than as a tuning parameter for which either systematic under- or overestimation is uniformly safe.
...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        ...
        \caption{\MPCchanged{\(\feps = 10^{-4}\).}}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../imgs/compare/_pp_eps_nominal_gtol1e-02.pdf}
        \caption{\MPCchanged{\(\feps = 10^{-2}\).}}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../imgs/compare/_pp_eps_over_gtol1e-02.pdf}
        \caption{\MPCchanged{\(\feps = 10^{-1}\).}}
    \end{subfigure}
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../imgs/compare/_legend_sensitivity.pdf}
    \end{subfigure}
    \caption{\MPCchanged{Performance profiles for error-bound misspecification under function-value-only noise of level $10^{-3}$ and \(\gtol = 10^{-2}\).}}
...
\begin{MPCchange}
At the strictest tolerance \(\gtol = 10^{-5}\), \texttt{Ours} and \texttt{Ours-MS} solve $85.5\%$ and $86.4\%$ of the 64-bit problems, $52.7\%$ and $52.3\%$ of the 32-bit problems, and $28.9\%$ and $28.9\%$ of the 16-bit problems.
...
Under exact gradients and uniformly bounded positive definite quasi-Newton matrices, the method retains the order-optimal \(\order{1/\varepsilon^2}\) first-order iteration complexity of the noise-free $L$-smooth nonconvex setting while selectively using noisy function values.
The implementation enforces the fixed-initial-scale upper-curvature condition on raw pairs, supplies the lower bound through a fixed diagonal shift, and uses the same non-restarted OFFO accumulator as the analysis.
