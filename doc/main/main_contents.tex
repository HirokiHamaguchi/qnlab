...
\begin{equation*}
    ...
    +(1-\theta_i^{\mathrm{damp}})B_{k-1}s_{k_i},
    \qquad \theta_i^{\mathrm{damp}}\in[0,1].
\end{equation*}
We determine \(\theta_i^{\mathrm{damp}}\) using the standard damped BFGS construction and its limited-memory variant~\citep{powellAlgorithmsNonlinearConstraints1978,lotfiStochasticDampedLBFGS2020} before applying \cref{eq:implemented_cautious_test}.
...
Each noisy experiment records the random seed, and all solvers receive the same seed for a given problem and scenario.
