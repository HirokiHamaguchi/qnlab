...
\MPCchanged{Set $\omega_{\min}\defeq\min\{1,\sqrt{\varsigma}\}>0$ and $\omega_k\defeq\max\{\omega_{\min},\norm{g_k}\}$.}
The third assumption requires the matrices $\{B_k\}_{k=0}^{k_{\mathrm{max}}-1}$ to be uniformly positive definite and \MPCchanged{upper bounded relative to $\omega_k$}.
\begin{assumption}\label{ass:BBounds}
    ...
    \begin{equation*}\label{eq:BBounds}
        mI \preceq B_k \preceq M\MPCchanged{\omega_k} I.
    \end{equation*}
\end{assumption}
...
Using $d_k=-(B_k+\mu_k I)^{-1} g_k$ (\refLine{line:d_k}) and \cref{ass:BBounds} asserting $(m + \mu_k)I \preceq B_k + \mu_k I \preceq (M\MPCchanged{\omega_k} + \mu_k)I$, we obtain the following bounds:
\begin{align}
    -g_k^\top d_k & = d_k^\top (B_k+\mu_k I) d_k      \ge (m+\mu_k)\norm{d_k}^2 \ge m \norm{d_k}^2, \label{eq:BBounds_results_zero} \\
    -g_k^\top d_k & = g_k^\top (B_k+\mu_k I)^{-1} g_k \ge \frac{1}{M\MPCchanged{\omega_k}+\mu_k}\norm{g_k}^2, \label{eq:BBounds_results_first}           \\
...
We now bound the sum of $\delta_k = f(x_k) - f(x_{k+1})$ over $k \in K^0$ with \MPCchanged{$\norm{g_{k}}^2/\omega_k$} as follows.
\begin{lemma}\label{lem:sum_small_delta_k_bound_K0}
    Under \cref{ass:bounded,ass:LSmooth,ass:BBounds}, the sum of $\delta_k$ over $k \in K^0$ satisfies
    \begin{equation*}
        \sum_{k \in K^0} \delta_{k} \geq \frac{c \overline{\alpha}}{M} \sum_{k \in K^0} \MPCchanged{\frac{\norm{g_{k}}^2}{\omega_k}} - \frac{2}{1-\feps} \qty(\overline{f}(x_0) - \overline{f}^* + \Delta_{\max}).
    \end{equation*}
\end{lemma}
\begin{proof}
...
\begin{equation*}
    \MPCchanged{D \defeq 1+\frac{M}{\theta_{\min}}, \qquad}
    \MPCchanged{C_1 \defeq \frac{\overline{\alpha}}{D\theta_{\max}}, \qquad}
    \MPCchanged{C_2 \defeq \frac{L}{2\theta_{\min}^2}.}
\end{equation*}
...
\begin{proof}
    ...
    \begin{align}
        \delta_k & = f(x_k) - f(x_k + \alpha_{k} d_{k})                                                          &  & (\text{\cref{eq:def_small_delta_k} and } x_{{k}+1}=x_{k} + \alpha_{k} d_{k})      \notag \\
                 ...
                 & \ge \qty(\frac{\alpha_{k}}{M\MPCchanged{\omega_k}+\mu_{k}} - \frac{\alpha_{k}^2 L}{2(m+\mu_{k})^2}) \norm{g_{k}}^2 &  & (\text{\cref{eq:BBounds_results_first,eq:BBounds_results_second}}) \notag                \\
                 & \ge \qty(\frac{\overline{\alpha}}{M\MPCchanged{\omega_k}+\mu_{k}} - \frac{L}{2(m+\mu_{k})^2}) \norm{g_{k}}^2.      &  & (\text{\cref{prop:backtracking_summary} and $\alpha_k \leq 1$})
        \label{eq:mu_positive_diff}
    \end{align}
    \MPCchanged{Since $\omega_{\min}\leq\sqrt{\varsigma}$ and the accumulator defining $\mu_k$ includes the current gradient, we have $\omega_k\leq\mu_k/\theta_{\min}$ for $k\in K^+$.}
    \MPCchanged{We can therefore bound the terms in \cref{eq:mu_positive_diff} as}
    \begin{equation}\label{eq:mu_positive_diff_bound}
        \MPCchanged{\frac{\overline{\alpha}}{M\omega_k+\mu_{k}} \geq \frac{\overline{\alpha}}{D\mu_k}}, \quad
    ...
    \begin{equation}\label{eq:mu_positive_diff_desired}
        \delta_k \ge \MPCchanged{\qty(\frac{\overline{\alpha}}{D\mu_{k}} - \frac{L}{2\mu_{k}^2})} \norm{g_{k}}^2.
    \end{equation}
    Summing \cref{eq:mu_positive_diff_desired} over $k \in K^+$ and applying \cref{lem:sum_norm_g_over_mu_geq_2sqrt_leq_log} yield
    \begin{align*}
              & \sum_{{k} \in K^+} \delta_k \ge \sum_{{k} \in K^+} \MPCchanged{\qty(\frac{\overline{\alpha}}{D\mu_{k}} - \frac{L}{2\mu_{k}^2})}\norm{g_{k}}^2                                                                                                 \\
        \ge{} & \MPCchanged{\frac{\overline{\alpha}}{D\theta_{\max}} \qty(\sqrt{\varsigma + \sum_{{k} \in K^+} \norm{g_{k}}^2} - \sqrt{\varsigma}) - \frac{L}{2\theta_{\min}^2} \qty(\ln\qty(\varsigma+\sum_{{k} \in K^+} \norm{g_{k}}^2) - \ln(\varsigma))},
    \end{align*}
...
\begin{lemma}\label{lem:sum_norm_g_k_bound}
    ...
    \begin{equation*}
        F \ge
        \frac{c \overline{\alpha}}{M} \sum_{k \in K^0} \MPCchanged{\frac{\norm{g_{k}}^2}{\omega_k}} +
...
\begin{proof}
    ...
    \begin{multline*}
        f(x_0) - f^*
        \ge \frac{c \overline{\alpha}}{M} \sum_{k \in K^0} \MPCchanged{\frac{\norm{g_{k}}^2}{\omega_k}}
...
\begin{proof}
    ...
    \begin{align*}
        F
         & \ge
        \frac{c \overline{\alpha}}{M} \sum_{k \in K^0} \MPCchanged{\frac{\norm{g_{k}}^2}{\omega_k}} + \frac{C_1}{2} \sqrt{\sum_{k \in K^+} \norm{g_{k}}^2} + \phi\qty(\sum_{k \in K^+} \norm{g_{k}}^2 + \varsigma) \\
         & \ge \frac{c \overline{\alpha}}{M} \sum_{k \in K^0} \MPCchanged{\frac{\norm{g_{k}}^2}{\omega_k}} + \frac{C_1}{2} \sqrt{\sum_{k \in K^+} \norm{g_{k}}^2} + \phi(G^\ast),
    \end{align*}
    which is equivalent to
    \begin{equation}\label{eq:final_inequality_for_gc}
        0 \leq \frac{c \overline{\alpha}}{M} \sum_{k \in K^0} \MPCchanged{\frac{\norm{g_{k}}^2}{\omega_k}} + \frac{C_1}{2} \sqrt{\sum_{k \in K^+} \norm{g_{k}}^2} \leq F'
    \end{equation}
    where
    \begin{align*}
        F' & \defeq F - \phi(G^\ast)                                                                                                                                                              \\
    ...
    Since $\omega_{\min}\leq1$, the definition of $\omega_k$ gives $\norm{g_k}^2/\omega_k>\varepsilon^2$ for $k\in K^0$.
...
\MPCchanged{The contribution of the indices in $K^0$ uses the scale-adjusted quantity $\norm{g_k}^2/\omega_k$, which equals $\norm{g_k}^2/\omega_{\min}$ near stationarity and therefore retains the classical first-order stationarity order.}
...
\end{equation*}
Here \(\overline\alpha\) depends only on \(c,L,m\) and the backtracking bounds, \(D=1+M/\theta_{\min}\), \(C_1=\overline\alpha/(D\theta_{\max})\), \(C_2=L/(2\theta_{\min}^2)\), and \(F'\) is displayed in the proof and contains \(\feps\), \(\varsigma\), the initial true and observed optimality gaps, and \(\Delta_{\max}\).
The construction in \cref{sec:proposed_choice_Bk} gives $m=\omega_{\min}$ and $M=1+(1+p)\Lambda$, so the memory length enters the sufficient iteration bound through $M$.
...
\MPCchanged{The purpose of this subsection is to show that a scaled L-BFGS construction with modified and carefully selected curvature pairs can satisfy \cref{ass:BBounds}.}
...
To make the upper test insensitive to the local gradient scale, define
\begin{equation*}
    \omega_{k_i}^{\mathrm{pair}}\defeq\max\{\omega_{\min},\norm{g_{k_i}},\norm{g_{k_i+1}}\},
    \qquad
    \widehat y_{k_i}\defeq\frac{\overline y_{k_i}}{\omega_{k_i}^{\mathrm{pair}}}.
\end{equation*}
After damping and normalization, a pair \((s,\widehat y)\) is retained only if
\begin{equation}\label{eq:implemented_cautious_test}
    \widehat y^\top s \geq \max\left\{\frac{\norm{\widehat y}^2}{10^8},10^{-16}\right\}.
\end{equation}
Thus every stored normalized pair satisfies \cref{eq:curvature_conditions1} with the iteration-independent constant \(\Lambda=10^8\).
The absolute floor rejects numerically degenerate pairs but is not needed for the upper bound.
Using at most $p=10$ pairs, define $\widehat B_k=\mathrm{BFGS}(\{(s_{k_i},\widehat y_{k_i})\})$ when the memory is nonempty and $\widehat B_k=I$ otherwise.
We then set
\begin{equation}\label{eq:proposed_choice_of_Bk}
    B_k=\omega_{\min}I+\omega_k\widehat B_k.
\end{equation}
By \cref{prop:bounded_BFGS}, $0\prec\widehat B_k\preceq(1+p)\Lambda I$ for nonempty memory, while the same upper bound holds for empty memory because $(1+p)\Lambda\geq1$.
Since \(\omega_{\min}\leq\omega_k\), it follows that
\begin{equation*}
    \omega_{\min}I\preceq B_k\preceq\qty(1+(1+p)\Lambda)\omega_kI.
\end{equation*}
Thus \cref{ass:BBounds} holds with the iteration-independent constants $m=\omega_{\min}$ and $M=1+(1+p)\Lambda$.
\end{MPCchange}
\begin{MPCchange}
...
Candidates for which this scalar damping is numerically unsafe are discarded before normalization and \cref{eq:implemented_cautious_test}.
This is the damping rule used in the released code; the normalized upper test and the fixed diagonal shift, rather than damping itself, guarantee the two bounds in \cref{ass:BBounds}.
...
\MPCchanged{Define $\widehat\mu_k\defeq(\mu_k+\omega_{\min})/\omega_k$.}
...
\begin{equation}\label{eq:approx_computation_of_regularized_quasi_Newton}
    \widehat B_k+\widehat\mu_k I
    \approx\mathrm{BFGS}(\{s_{k_i},\widehat y_{k_i}+\widehat\mu_k s_{k_i}\}_{i=0}^{p-1}).
\end{equation}
\end{MPCchange}
\MPCchanged{The direction is then computed as $d_k\approx-\omega_k^{-1}\mathrm{BFGS}(\{s_{k_i},\widehat y_{k_i}+\widehat\mu_k s_{k_i}\})^{-1}g_k$ using the standard two-loop recursion~\citep{liuLimitedMemoryBFGS1989,nocedal1999numerical}.}
...
Under exact gradients, a fixed positive lower spectral bound, and a gradient-scaled upper spectral bound on the quasi-Newton matrices, the method retains the order-optimal \(\order{1/\varepsilon^2}\) first-order iteration complexity of the noise-free $L$-smooth nonconvex setting while selectively using noisy function values.
The implementation enforces the normalized upper-curvature condition, supplies the lower bound through a fixed diagonal shift, and uses the same non-restarted OFFO accumulator as the analysis.
