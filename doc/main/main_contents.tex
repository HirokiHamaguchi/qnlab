...
\begin{proof}
    ...
    \begin{align*}
        \overline{\delta}_{k} + \Delta_{k}
         ...
         & = \frac{c \overline{\alpha}}{M} \norm{g_{k}}^2.            &  & (\mu_{k} = 0 \text{ since } k \in K^0)
    \end{align*}
    ...
    \end{equation*}
    Then, combining these inequalities and summing over $k \in K^0$ gives
    \begin{equation*}
...
For each problem $p \in P$ and solver $s \in S$, let $n_{p,s} > 0$ denote the number of function and gradient evaluations required to reach a specified gradient norm tolerance $\gtol$.
...
In this regime, the proposed methods show superior robustness compared to other quasi-Newton methods.
...
The proposed methods maintain stability even when traditional line search methods encounter difficulties with reduced precision arithmetic.
