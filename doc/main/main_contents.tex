...
\begin{MPCchange}
For the primary NTQN comparison, we use the wrapper's common gradient-tolerance stopping rule.
We additionally run the authors' recommended termination mode (\texttt{terminate=3}) with that wrapper-level stop disabled.
In the function-noise experiment, NTQN receives the conservative bound $\texttt{eps\_f}=10^{-2}$ for perturbations bounded by $10^{-3}$, matching the value of $\feps$ supplied to the proposed method.
NTQN uses this bound in both its noise-tolerant line search and its recommended termination test.
In the recommended mode, termination is controlled by NTQN itself, while the common gradient tolerance is applied only afterward to its recorded trajectory for uniform scoring.
Because NTQN may appropriately stop at its estimated noise neighborhood before attaining that tolerance, we report both solver-declared termination and attainment of the common scoring tolerance and distinguish such early termination from poor oracle efficiency.
\end{MPCchange}
...
Across the $220$ problems and five paired seeds, \texttt{Ours} and \texttt{Ours-MS} attain \(\gtol = 10^{-2}\) on $85.1\%$ and $85.5\%$, respectively, of the $1{,}100$ function-noise instances.
Their seed-to-seed standard deviations in the solved fraction are only $0.60$ and $0.18$ percentage points.
...
Among successful runs, the median oracle-call counts are $62$ for \texttt{Ours}, $67$ for \texttt{Ours-MS}, $64$ for \texttt{NTQN}, and $131$ for \texttt{ASTR1-Adagrad}.
The shifted-pair variant \texttt{Ours-SP} solves $83.9\%$, while the compact solver records no fallback for \texttt{Ours} and nine fallbacks in total for \texttt{Ours-MS} across these instances.
...
In the joint-noise stress test, \texttt{Ours} and \texttt{Ours-MS} solve $83.6\%$ and $84.8\%$ of the instances, compared with $71.9\%$ for \texttt{NTQN}, $73.5\%$ for \texttt{Reg}, and $42.8\%$ for \texttt{ASTR1-Adagrad}.
The two proposed rates have seed-to-seed standard deviations of $0.64$ and $0.22$ percentage points.
The corresponding \texttt{Ours-SP} rate is $80.6\%$, and the compact solver records two and one fallbacks for \texttt{Ours} and \texttt{Ours-MS}, respectively.
...
\begin{MPCchange}
For the error-bound sensitivity study, underestimating \(\feps\) as $10^{-4}$ gives solved fractions of $87.1\%$ for \texttt{Ours} and $81.9\%$ for \texttt{Ours-MS}, whereas the nominal $10^{-2}$ gives $85.1\%$ and $85.5\%$, and overestimating it as $10^{-1}$ gives $75.5\%$ and $74.7\%$.
Thus, underestimation happens to improve the aggregate success rate of the unmodified variant but harms the modified-secant variant, while the tenfold overestimate reduces the rates by $9.6$ and $10.8$ percentage points, respectively, relative to the nominal setting.
The relatively high solved fractions remain plausible because this experiment uses exact gradients, its termination tolerance $\gtol = 10^{-2}$ exceeds the function-noise level by one order of magnitude, and function-value comparisons remain informative when the actual decrease dominates the noise.
...
\begin{MPCchange}
In \cref{fig:exec_time}, we report the execution time statistics: the shifted-pair \texttt{Ours-SP} has per-iteration cost close to the least expensive Python implementations, whereas the exact compact solves in \texttt{Ours} and \texttt{Ours-MS} take about $1.8$ times its runtime in this isolated test.
