  ...
  \begin{table}
    ...
    \begin{tabular}{m{0.5cm}m{0.5cm}|C{0.4\columnwidth}C{0.4\columnwidth}}
      ...
      \multirow{2}{*}{\rotatebox{90}{$\nabla f$ の誤差}}
       ...
       & \makecell[c]{通常の最適化                                                                                                          \\ \citep{liuLimitedMemoryBFGS1989,2020SciPy-NMeth}}
...
\begin{frame}{研究概要}
  ...
  \begin{exampleblock}{先行研究}
    \begin{itemize}
      \item 直線探索型準ニュートン法\citep{liuLimitedMemoryBFGS1989,2020SciPy-NMeth}は高速だが脆弱
...
\begin{frame}{理論解析における仮定}
  ...
  \begin{block}{仮定1: 目的関数 $f$ の下側に関する有界性}
    ...
    \end{equation*}
  \end{block}
  \begin{block}{仮定2: 目的関数 $f$ の $L$-平滑性}
    \begin{equation*}
...
\begin{frame}{収束解析の概要: コアとなる不等式}
  \vspace{-0.7em}
  \begin{equation*}
    K^0 = \{k \in K \mid \mu_k = 0\}, \; K^+ = \{k \in K \mid \mu_k > 0\}, \; (\text{Armijo定数}) c, \; (\text{平滑性}) L.
  \end{equation*}
  \vspace{-0.7em}
...
\begin{frame}{実験設定}
  \begin{block}{比較手法}
    \begin{itemize}[leftmargin=4em]
      ...
      \item[\texttt{Line}:] Wolfe条件による直線探索型
...
\begin{frame}{有限精度計算に対する頑健性}
  \begin{columns}
    ...
    \centering
    32-bit $\gtol=10^{-3}$\\
    \includegraphics[width=\linewidth]{../imgs/compare/_pp_precision32_gtol1e-03.pdf}

    \column{0.32\textwidth}
    \centering
    16-bit $\gtol=10^{-1}$\\
    \includegraphics[width=\linewidth]{../imgs/compare/_pp_precision16_gtol1e-01.pdf}
...
\begin{frame}{誤差なし設定からみる実用性}
  \begin{columns}
    ...
    \begin{center}
      誤差の殆ど関係ない設定:\\
      64-bit $\gtol=10^{-1}$\\
      \includegraphics[width=\columnwidth]{../imgs/compare/_pp_precision64_gtol1e-01.pdf}
      最速で解けた問題数が、\\
      2倍近くに達している
    \end{center}
    \hfill
