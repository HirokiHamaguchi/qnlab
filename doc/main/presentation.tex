...
\begin{frame}{無制約最適化問題と準ニュートン法の一種 L-BFGS}
    \begin{itemize}
        \item 問題設定: $\min_{x \in \mathbb{R}^n} f(x)$ \quad ($f\colon \mathbb{R}^n \to \mathbb{R}$ は連続可微分、一般には非凸)
        \item 問題が解ける: $\norm{\nabla \overline{f}(x)} \leq \gtol$ を満たす点 $x$ の発見
...
\begin{frame}{問題意識: 数値誤差に対する脆弱性}
    \begin{enumerate}[label=\textbullet]
        ...
        \item 特に、\textbf{関数値}の誤差は準ニュートン法の性能を著しく劣化させる
...
\begin{frame}{本研究の位置づけ}
    \begin{itemize}
        \item 無視できない数値誤差が $f$ にも $\nabla f$ にも存在すると、その最適化問題は不良設定問題
        \item 本研究は、特に\underline{関数値に誤差が存在する}場合の最適化手法に焦点を当てる
    \end{itemize}
    \begin{table}
        \centering
        ...
        \end{tabular}
    \end{table}
    より緩い誤差を扱ったり\citep{sunTrustRegionMethod2023,paquetteStochasticLineSearch2020,byrdSampleSizeSelection2012}、\\
    信頼領域法における研究例\citep{sunTrustRegionMethod2023,bellaviaImpactNoiseEvaluation2021}も存在\\
    $\to$ 本研究は、\underline{消すことが出来ない誤差}に対して\underline{実用的な高速性を維持}したい
\end{frame}

\begin{frame}{問題に対する仮定}
    停止条件は $\norm{\nabla \overline{f}(x)} \leq \gtol$ 最適化過程では常に $\norm{\nabla \overline{f}(x)} > \gtol$ が成立

    \begin{block}{勾配に対する誤差モデル: 誤差が $\gtol$ より十分小さい}
        \begin{equation*}
            \text{(数理実験)} \norm{\nabla f(x)-\nabla\overline{f}(x)}\ll\gtol, \qquad
            \text{(理論解析)} \norm{\nabla f(x)-\nabla\overline{f}(x)}=0.
        \end{equation*}
    \end{block}
    \begin{exampleblock}{関数値に対するハイブリッド型の誤差モデル}
        ...
        \end{equation*}
    \end{exampleblock}
    現実的なIEEE~754浮動小数点数の丸め誤差モデルを想定した問題設定
\end{frame}
...
\begin{frame}{研究概要}
    ...
    \begin{alertblock}{本研究の貢献}
        \begin{itemize}
            \item 正則化準ニュートン法を用いながら、\underline{数値誤差を陽に考慮する}
            \item 関数値の誤差問題を回避するために\underline{関数値非依存最適化}を援用
...
\begin{frame}{キーワード:目的関数値非依存最適化 \citep{grattonOFFOMinimizationAlgorithms2023}}
    ...
    \Adam や \AdaGrad などの最適化手法は、勾配だけで最適化を行う。
    \begin{equation*}
        x_{k+1}^{(i)} = x_k^{(i)} - \frac{1}{\theta\sqrt{\varsigma + \sum_{j=0}^k (g_j^{(i)})^2}} g_k^{(i)}, \
        x_{k+1} = x_k - \frac{1}{\theta\sqrt{\varsigma + \sum_{j=0}^k \norm{g_j}^2}}\,g_k.
    \end{equation*}
    ($g_k$ は勾配, $\theta$ は学習率, $\varsigma$ は初期化定数)\\
    \vspace{1em}
    このような\underline{関数値}を陽に用いない最適化手法の総称。\\
    $\to$ 誤差に影響されやすい\underline{関数値差分}を利用せずに最適化が可能。
\end{frame}

\begin{frame}{提案手法の構成要素1/2: 緩和Armijo条件}
    通常のArmijo条件:
    \begin{equation*}
        \overline{f}(x_k) + c \alpha_k g_k^\top d_k \ge \overline{f}(x_k + \alpha_k d_k).
    \end{equation*}

    本研究は以下の緩和 Armijo 条件を採用:
    \begin{equation*}
        \begin{dcases}
            \overline{f}(x_k) + c \alpha_k g_k^\top d_k + \red{\Delta_k} \ge \overline{f}(x_k + \alpha_k d_k), \\
...
\begin{frame}{提案手法の構成要素2/2: 正則化を入れた探索方向}
    ...
    \end{equation*}
    (直感: $f$ の勾配情報による近似 + 正則化項 $\frac{\mu_k}{2}\norm{d}^2$ を加えた二次形式の最小解方向)
    \begin{itemize}
        \item $\mu_k=0$: 直線探索型とほぼ同じ挙動を意味、速度の観点からは望ましい
        \item $\mu_k>0$: 保守的なステップになり、安定性向上、 \AdaGradNorm に似た正則化:
              \begin{equation*}
                  \mu_k = \theta_k \sqrt{\varsigma + \sum_{j \in K^+} \norm{g_j}^2},\quad \theta_k \in [\theta_{\min}, \theta_{\max}].
              \end{equation*}
...
\begin{frame}{本質的な設計思想の図示}
    \begin{itemize}
        ...
        \item $\mu_k=0$ を優先的に試すが、十分に $f$ が減少した試行に限ることで、安定化 (図右側)
    \end{itemize}
    \begin{figure}[htbp]
        \centering
...
\begin{frame}{理論解析における仮定}
    ...
    \begin{block}{仮定2: 目的関数 $f$ の$L$-smooth性}
        \begin{equation*}
            \exists\, 0< L < \infty \text{ s.t. } f(y) \le f(x) + \nabla f(x)^\top (y - x) + \frac{L}{2} \norm{y - x}^2.
        \end{equation*}
    \end{block}
\end{frame}
...
\begin{frame}{収束解析の概要: コアとなる不等式}
    ...
    \begin{block}{勾配ノルム和に対する複合的な上界評価}
        仮定1,2,3と
        $F = \order{f(x_0)-f^*}$,
        $C_0 = \order{\frac{2c(1-c)}{L}}$,
        $C_1, C_2 = \order{\frac{2(1-c)}{L}}$
        を用いて、\vspace{-0.5em}
        \begin{equation*}
            F \ge C_0 \sum_{k\in K^0}\|g_k\|^2 + C_1 \sqrt{\varsigma+\sum_{k\in K^+}\norm{g_k}^2} - C_2\ln\Bigl(\varsigma+\sum_{k\in K^+}\norm{g_k}^2\Bigr).
        \end{equation*}
    \end{block}
    \vspace{-1.2em}
    \begin{columns}[t]
        ...
        \begin{exampleblock}{$K^+$:正則化が入る保守的ステップ}
            ...
            \end{itemize}
            \[
                \sum_{k\in K^+}\norm{g_k}^2 \lesssim \qty(\frac{2F}{C_1})^2.
            \]
        \end{exampleblock}
    \end{columns}
\end{frame}
...
\begin{frame}{大域収束率の結論}
    \begin{block}{主結果}
        仮定1,2,3の下で、提案手法は以下を満たす:
        \[
            \sum_{j=0}^{k_{\max}} \norm{g_j}^2 \le \text{const.}
            \implies
            \frac{1}{k_{\max}+1}\sum_{j=0}^{k_{\max}} \norm{g_j}^2 = \order{\frac{1}{k_{\max}}}.
        \]
    \end{block}
    \begin{exampleblock}{反復計算量}
        \[
    ...
    \end{exampleblock}
    \begin{itemize}
        \item \underline{誤差がある設定}でも、非凸問題に対する\underline{最適な反復計算量}とオーダーレベルで一致
        \item 具体的には、古典的な勾配法型の解析結果と \AdaGradNorm 型の解析結果\\
              のハイブリッドになっている
    \end{itemize}
\end{frame}
...
\begin{frame}{実験設定}
    \begin{block}{比較手法}
        \begin{itemize}
            ...
            \item \texttt{SciPy}: SciPy L-BFGS-B (冒頭で見せた著名OSSの実装)
            \item \texttt{Reg}: 正則化 L-BFGS \citep{kanzowRegularizationLimitedMemory2023}
            \item \texttt{NTQN}: ノイズ耐性準ニュートン法 \citep{shiNoiseTolerantQuasiNewtonAlgorithm2022} (冒頭で述べた0次法に類似した手法)
        \end{itemize}
    \end{block}
    \begin{columns}
        ...
        \begin{column}{0.55\columnwidth}
            \begin{block}{評価手法: Performance Profile}
                \begin{itemize}
...
\begin{frame}{人工ノイズ下での性能}
    \begin{block}{ノイズモデル \citep{shiNoiseTolerantQuasiNewtonAlgorithm2022}}
        \[
    ...
    \begin{columns}
        ...
        \begin{column}{0.5\columnwidth}
            \begin{itemize}
                ...
                \item 先行研究(\texttt{NTQN}) は勾配ノイズに強いが、我々の手法も勾配ノイズに耐性
...
\begin{frame}{誤差なし設定からみる実用性}
    \begin{columns}
        ...
        \begin{itemize}
            ...
            \item 実装における既存手法などの工夫を活用
            \item 特に、仮定3を満たすためにdamped BFGSなどを採用
                  \begin{align*}
                      B_k & =\mathrm{BFGS}(\{(s_{k_i}, \overline{y}_{k_i} \defeq \theta^{\mathrm{damp}}_i y_{k_i} + (1-\theta^{\mathrm{damp}}_i) B_{k-1}s_{k_i})\}), \\
                          & (\text{$\mathrm{BFGS}(\cdot)$ は行列を返す関数、$s_{k_i}, y_{k_i}$ は差分ベクトル})
                  \end{align*}
            \item 既存の理論的に良いとされる正則化に類似点
                  \begin{equation*}
                      \mu_k ={}   c_1 \max(0, -\lambda_{\min}(B_k)) + c_2 \norm{\nabla f(x_k)}^\delta \quad (c_1, c_2, \delta > 0)
                  \end{equation*}
        \end{itemize}
    \end{columns}
\end{frame}
...
\begin{frame}{実行時間からみる実用性}
    ...
    \begin{align*}
        ...
        \text{(探索方向)}    &  & d_k & =-(B_k+\mu_k I)^{-1}g_k = -\mathrm{BFGS}(\dots)^{-1}g_k.
    \end{align*}
\end{frame}
