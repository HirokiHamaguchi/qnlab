...
\begin{frame}{キーワード:目的関数値非依存最適化 \citep{grattonOFFOMinimizationAlgorithms2023}}
    ...
    \begin{align*}
        (\text{\Adam}) \quad x_{k+1}^{(i)} & = x_k^{(i)} - \frac{1}{\theta\sqrt{\varsigma + \sum_{j=0}^k (\nabla f(x_j)^{(i)})^2}} \nabla f(x_k)^{(i)}, \\
        (\text{\AdaGrad}) \quad x_{k+1}    & = x_k - \frac{1}{\theta\sqrt{\varsigma + \sum_{j=0}^k \norm{\nabla f(x_j)}^2}}\,\nabla f(x_k).
    \end{align*}
    ($\theta$ は学習率, $\varsigma$ は初期化定数, ${}^{(i)}$ は第 $i$ 成分)\\
    \vspace{2em}
...
\begin{frame}{大域収束率の結論}
    \begin{block}{主結果}
        仮定1,2の下で、任意の反復回数 $k_{\max}$ に対し、提案手法は以下を満たす:
        \[
